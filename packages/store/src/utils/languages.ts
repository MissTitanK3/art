...
export function normalizeLanguage(inputRaw: string): NormalizedLanguage | null {
  ...
  if (/^portuguese.*brazil/.test(key))
    return { tag: "pt-BR", display_name: "Portuguese (Brazil)" };
  if (/^portuguese.*portugal/.test(key))
    return { tag: "pt-PT", display_name: "Portuguese (Portugal)" };
  if (/^arabic.*egypt/.test(key))
    return { tag: "ar-EG", display_name: "Arabic (Egypt)" };
  if (/^chinese.*simplified/.test(key))
    return { tag: "zh-Hans", display_name: "Chinese (Simplified)" };
  if (/^chinese.*traditional/.test(key))
    return { tag: "zh-Hant", display_name: "Chinese (Traditional)" };
