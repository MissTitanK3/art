...
USING (
  (EXISTS (
    ...
    WHERE ((p.user_id = (auth.uid())::text)
       AND (p.access_role = ANY (ARRAY['dispatcher_verified'::text, 'dispatcher_admin'::text]))))
  )
)
WITH CHECK (
  (EXISTS (
...
USING (
  ...
  OR (EXISTS (
    SELECT 1 FROM profiles p
    WHERE ((p.user_id = (auth.uid())::text)
       AND (p.access_role = ANY (ARRAY['dispatcher_basic'::text,'dispatcher_verified'::text,'dispatcher_admin'::text]))))
  )
);
...
USING (
  (EXISTS (
    SELECT 1 FROM profiles p
    WHERE ((p.user_id = (auth.uid())::text)
       AND p.access_role = 'dispatcher_admin'))
  )
)
WITH CHECK (
  (EXISTS (
