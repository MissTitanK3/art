...
export function formatDistance(valueKm: number, unit: DistanceUnit, digits = 1): string {
  const v = unit === 'mi' ? kmToMi(valueKm) : valueKm;
  return `${Number(v.toFixed(digits))} ${unit}`;
}

export function normalizeDistanceToKm(value: number, unit: DistanceUnit): number {
