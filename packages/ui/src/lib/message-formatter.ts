...
export function generateMessages(dispatch: DispatchSubmission, urgency: string) {
  ...
  // Concise public-facing summary for compact cards
  const rawSummary = dispatch.summary?.trim();
  const shortSummary = rawSummary ? (rawSummary.length > 140 ? `${rawSummary.slice(0, 137)}...` : rawSummary) : null;
  ...
  // Include public summary alongside intended actions where applicable
  const actionsWithOptionalSummary = () => {
  ...
  const detailedSections = [
    ...
    },
    {
      ...
      body: actionsWithOptionalSummary(),
    },
    {
      title: '💡 Roles Needed',
  ...
  const volunteerCalloutSections = [
    ...
    },
    {
      title: '👥 Volunteers',
      body: `${volunteerLine}\n\n📲 How to Join:\n${
        actionSignalLink ? 'Join the Signal group linked above if available' : 'Reply in this group if available'
      }`,
    },
  ];

  const volunteerCallout = volunteerCalloutSections.map((s) => `${s.title}\n${s.body}`).join('\n\n');
...
`.trim(),
    ...
    tldr: `${emoji} ${cityState}: ${volunteerLine}. Roles: ${formatRoles(dispatch).replace(
      /\n/g,
      ', ',
    )}. ${dispatch.summary ? `Summary: ${dispatch.summary}. ` : ''}📲 Reply in this group if available.`,

    // --- TL;DR section for social cards ---
    tldrSection: {
      title: `${emoji} ${cityState.toUpperCase()}`,
      body: `🕓 ${urgency}${shortSummary ? `\n\n${shortSummary}` : ''}\n\n👥 ${volunteerLine}\n\n📲 Join our public chat`,
    },
  };
}
