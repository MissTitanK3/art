...
export const TELEPROMPTER_PRESETS: Record<
  PresetId,
...
export const closestSpeedPresetId = (v: number): SpeedPresetId => {
  const entries = Object.entries(SPEED_PRESETS) as Array<
    [SpeedPresetId, { label: string; value: number }]
  >;
...
export const computeLineMs = (
  line: string,
  spd: number,
  baseMsPerChar: number = BASE_MS_PER_CHAR,
) => {
  let ms = Math.max(
    300,
    (Math.max(1, (line ?? "").trim().length) * baseMsPerChar) /
      Math.max(0.25, spd),
  );
...
export const estimateTotalMs = (
  ...
  const totalChars = lines.reduce(
    (acc, l) => acc + Math.max(1, l.trim().length),
    0,
  );
...
export const computeLineMsOrdered = (
  line: string,
  spd: number,
  cfg: SegmentConfig = {},
) => segmentsForLine(line, spd, cfg).reduce((a, s) => a + s.durationMs, 0);

export const estimateTotalMsOrdered = (
  text: string,
  spd: number,
  cfg: SegmentConfig = {},
) => {
