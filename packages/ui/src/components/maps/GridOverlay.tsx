...
const gridCache = new Map<
  string,
  { cells: { id: number; coords: L.LatLngTuple[][] }[] }
>();
...
export default function GridOverlay({
  ...
  const [countyFeature, setCountyFeature] = useState<Feature<
    Polygon | MultiPolygon
  > | null>(null);
  const [gridCells, setGridCells] = useState<
    { id: number; coords: L.LatLngTuple[][] }[]
  >([]);
  ...
  useEffect(() => {
    ...
    fetch("/us-counties.json")
      ...
      .then((geo: FeatureCollection<Polygon | MultiPolygon>) => {
        ...
        const feature =
          geo.features.find(
            (f) => f.properties?.GEO_ID === selectedCounty.GEO_ID,
          ) || null;
  ...
  useEffect(() => {
    ...
    const idleId = requestIdle(() => {
      ...
      const hexGrid = turf.hexGrid([minX, minY, maxX, maxY], hexSizeKm, {
        units: "kilometers",
      });
      const cells: { id: number; coords: L.LatLngTuple[][] }[] = [];

      const hexes = hexGrid.features.filter((h) =>
        turf.booleanIntersects(h, cf),
      );
      const batchSize = 200;
      let index = 0;
          ...
          if (clipEdges) {
            try {
              const clipped = turf.intersect(
                turf.featureCollection([cf, hex]) as FeatureCollection<
                  Polygon | MultiPolygon
                >,
              );
  ...
  const maskCoords = useMemo(() => {
    ...
    const cleanedCounty = turf.rewind(cf, { reverse: true }) as Feature<
      Polygon | MultiPolygon
    >;

    try {
      const mask = turf.difference(
        turf.featureCollection([worldPoly, cleanedCounty]) as FeatureCollection<
          Polygon | MultiPolygon
        >,
      );
      if (!mask) return null;
              ...
              <LeafletPolygon
                ...
                eventHandlers={{
                  ...
                  mousemove: () => {
                    if (!paintingRef.current) return;
                    const next = new Set(zonesRef.current);
                    if (eraseModeRef.current) next.delete(cell.id);
