...
export function AdminNotificationForm({ onSend }: { onSend: (args: SendArgs) => void }) {
  ...
  const [profiles, setProfiles] = React.useState<Pick<Profile, 'id' | 'user_id' | 'display_name' | 'access_role'>[]>([]);
  ...
  const [selectedUsers, setSelectedUsers] = React.useState<Pick<Profile, 'user_id' | 'display_name'>[]>([]);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await fetch('/api/admin/profiles', { credentials: 'include', signal: controller.signal });
        ...
        if (!cancelled) {
          setProfiles(
            list
              .filter((p) => !!p.user_id)
              .map((p) => ({ id: p.id, user_id: p.user_id, display_name: p.display_name, access_role: p.access_role })),
          );
        }
      } catch (e: any) {
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between align-baseline">
        <div className="flex gap-3 flex-col md:flex-row md:items-end">
          ...
          <div>
            ...
            <Input className="w-56" value={channel} onChange={(e) => setChannel(e.target.value)} aria-labelledby="channel-label" />
          </div>
          <div>
            <span id="link-label" className="text-sm font-medium">Link</span>
            <Input className="w-72" value={link} onChange={(e) => setLink(e.target.value)} aria-labelledby="link-label" />
          </div>
          <div className="flex items-center gap-2 mt-1.5">
            <Switch checked={sticky} onCheckedChange={setSticky} id="sticky" />
            <label htmlFor="sticky" className="text-sm">Sticky</label>
          </div>
          <div>
            <span id="ttl-label" className="text-sm font-medium">TTL (minutes)</span>
            <Input
              className="w-36"
              value={ttlMinutes}
              onChange={(e) => {
      ...
      <div className="grid gap-2">
        ...
        <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Subject/title of the notification" />
      </div>
      <div className="grid gap-2">
      ...
      <div className="grid gap-2">
        ...
        <div className="text-xs text-muted-foreground">Select specific users (optional)</div>
        <div className="flex flex-col gap-2">
          <Popover>
            <PopoverTrigger asChild>
              <Button type="button" variant="outline" className="min-w-64 justify-between">
                {selectedUsers.length === 0 ? "Select users" : `${selectedUsers.length} user${selectedUsers.length === 1 ? '' : 's'} selected`}
                <ChevronDown className="h-4 w-4" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="p-0 w-[420px]">
              <Command>
                <CommandInput placeholder={profilesLoading ? "Loading users..." : "Search users"} disabled={profilesLoading} />
                <CommandList>
                  {profilesError ? (
                    <CommandEmpty>{profilesError}</CommandEmpty>
                  ) : (
                    <>
                      <CommandEmpty>{profilesLoading ? 'Loading…' : 'No users found'}</CommandEmpty>
                      <CommandGroup>
                        {profiles.map((p) => {
                          const checked = !!selectedUsers.find((u) => u.user_id === p.user_id);
                          const value = `${p.display_name ?? ''} ${p.user_id ?? ''} ${p.access_role ?? ''}`.trim();
                          return (
                            <CommandItem key={p.user_id} value={value} onSelect={() => toggleUser({ user_id: p.user_id, display_name: p.display_name })}>
                              <div className="flex items-center gap-2 w-full justify-between">
                                <div className="flex flex-col">
                                  <span className="font-normal">{p.display_name || p.user_id}</span>
                                  <span className="text-xs text-muted-foreground">{p.access_role ? `${humanize(p.access_role)} • ` : ''}{p.user_id}</span>
                                </div>
                                <Checkbox checked={checked} onCheckedChange={() => toggleUser({ user_id: p.user_id, display_name: p.display_name })} />
                              </div>
                            </CommandItem>
                          );
                        })}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {selectedUsers.map((u) => (
              <Badge key={u.user_id} variant="outline" className="flex items-center gap-1">
      ...
      <div className="grid gap-2">
        ...
        <div className="text-xs text-muted-foreground">Select specific roles or use a preset below</div>
        <div className="flex flex-wrap gap-2">
          <Popover>
            <PopoverTrigger asChild>
              <Button type="button" variant="outline" className="min-w-48 justify-between">
                {selectedRoles.length === 0 ? "Select roles" : `${selectedRoles.length} roles selected`}
                <ChevronDown className="h-4 w-4" />
              </Button>
            </PopoverTrigger>
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    {AccessRoles.map((r) => {
                      ...
                      return (
                        <CommandItem key={r} onSelect={() => setRoleChecks((prev) => ({ ...prev, [r]: !checked }))}>
                          <div className="flex items-center gap-2">
                            <Checkbox checked={checked} onCheckedChange={(v) => setRoleChecks((prev) => ({ ...prev, [r]: !!v }))} />
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {selectedRoles.map((r) => (
              <Badge key={r} variant="outline" className="flex items-center gap-1">
                {roleLabel(r as any)}
                <button type="button" onClick={() => setRoleChecks((prev) => ({ ...prev, [r]: false }))}>
                  <X className="h-3 w-3" />
                </button>
              </Badge>
            ))}
        ...
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="secondary" size="sm" onClick={() => applyPreset('verified_dispatchers')}>Verified Dispatchers</Button>
