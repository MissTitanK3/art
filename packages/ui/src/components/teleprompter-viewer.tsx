...
import { TELEPROMPTER_PRESETS, PresetId, parseCues } from "@workspace/ui/lib/teleprompter";

export type TeleprompterTheme = { cls?: string; highlightCls?: string; nextCls?: string } | PresetId;

type FontFace = "sans" | "serif" | "mono" | "dyslexic";

type CountdownSegment = { name: "base" | "pause" | "breathe" | "lookup" | "custom"; durationMs: number };
...
export type TeleprompterViewerProps = {
  ...
  showLegend?: boolean;
  countdown?: { enabled: boolean; totalMs?: number; startedAt?: number; dotMs?: number; segments?: CountdownSegment[] };
...
const useCountdownDots = (enabled: boolean, totalMs?: number, startedAt?: number, dotMs = 1000) => {
  ...
  const DOTS_MIN = 3;
  const DOTS_MAX = 12;
  const totalDots = Math.max(DOTS_MIN, Math.min(DOTS_MAX, Math.round(totalMs / dotMs)));
...
export const TeleprompterViewer = React.forwardRef<HTMLDivElement, TeleprompterViewerProps>(function TeleprompterViewer(
  {
    text,
    index,
    onIndexRef,
  ...
  const activeDotsColor = useActiveCountdownColor(!!countdown?.enabled, countdown?.startedAt, countdown?.totalMs, countdown?.segments);

  const lines = React.useMemo(() => (text ?? "").split(/\r?\n/), [text]);
  const preset = typeof theme === "string" ? TELEPROMPTER_PRESETS[theme] : theme;
  const clsBase = typeof theme === "string" && theme !== "custom" ? preset.cls : "";
  const highlightCls = typeof theme === "string" && theme !== "custom" ? preset.highlightCls : "";
  const nextCls = typeof theme === "string" && theme !== "custom" ? preset.nextCls : "opacity-80";
  ...
  React.useEffect(() => {
    ...
    const targetTop = container.scrollTop + offset - container.clientHeight / 2 + el.clientHeight / 2;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        ref={scrollRef}
        style={{ ...fontFamilyFor(font?.face), overscrollBehavior: "contain", WebkitOverflowScrolling: "touch", wordBreak: "break-word" }}
      >
        {showLegend && (
          <div className="mb-2 rounded-md border bg-muted/40 p-3 text-sm">
            <div className="flex flex-wrap items-center gap-3">
              <span className="inline-flex items-center gap-1 rounded bg-amber-500/20 px-2 py-0.5 text-amber-300">[pause] adds ~{legendDurations?.pauseMs ?? 600}ms</span>
              <span className="inline-flex items-center gap-1 rounded bg-emerald-500/20 px-2 py-0.5 text-emerald-300">[breathe] adds ~{legendDurations?.breatheMs ?? 300}ms</span>
              <span className="inline-flex items-center gap-1 rounded bg-sky-500/20 px-2 py-0.5 text-sky-300">[look up] adds ~{legendDurations?.lookupMs ?? 1200}ms</span>
              <span className="inline-flex items-center gap-1 rounded bg-fuchsia-500/20 px-2 py-0.5 text-fuchsia-300">[custom] adds ~250ms</span>
            </div>
          </div>
        )}
        {lines.map((ln, i) => {
              ...
              {ln?.length ? (
                <span className="inline">
                  {isCurrent ? (
                    <>
                      <span className="ml-2 inline-flex items-center gap-2 align-baseline">
                        <span className={`text-sm tracking-widest select-none ${activeDotsColor ?? "text-muted-foreground/70"}`} aria-hidden>
                          {dots}
                        </span>
                        {/* Inline upcoming cues preview for current line */}
                        <span className="text-[10px] text-muted-foreground/70 hidden md:inline-flex items-center gap-1" aria-hidden>
                          {(() => {
                            const chunks = parseCues(ln);
                            const items: React.ReactNode[] = [];
                            for (const c of chunks) {
                              if (c.t === "pause") items.push(<span key={items.length} className="inline-flex items-center gap-1 rounded bg-amber-500/20 px-1.5 py-0.5 text-amber-300">pause</span>);
                              if (c.t === "breathe") items.push(<span key={items.length} className="inline-flex items-center gap-1 rounded bg-emerald-500/20 px-1.5 py-0.5 text-emerald-300">breathe</span>);
                              if (c.t === "lookup") items.push(<span key={items.length} className="inline-flex items-center gap-1 rounded bg-sky-500/20 px-1.5 py-0.5 text-sky-300">look up</span>);
                            }
                            return items.length ? <span className="inline-flex items-center gap-1">{items}</span> : null;
                          })()}
                        </span>
                      </span>
                        ...
                        <div className="mt-1 h-1.5 w-full max-w-xs overflow-hidden rounded bg-muted justify-center mx-auto">
                          <div className="flex h-full w-full">
                            {countdown.segments.map((seg, i) => {
                              const color = seg.name === 'pause' ? 'bg-amber-500/70' : seg.name === 'breathe' ? 'bg-emerald-500/70' : seg.name === 'lookup' ? 'bg-sky-500/70' : seg.name === 'custom' ? 'bg-fuchsia-500/70' : 'bg-muted-foreground/40';
                              const widthPct = Math.max(0, Math.min(100, Math.round(((seg.durationMs || 0) / Math.max(1, (countdown.totalMs || 1))) * 100)));
                              return <div key={i} className={`${color} h-full`} style={{ width: `${widthPct}%` }} />
                            })}
                          </div>
                        </div>
                      ) : null}
                  ...
                  {parseCues(ln).map((chunk, idx) => {
                    if (chunk.t === "text") return <span key={idx}>{chunk.v}</span>;
                    if (chunk.t === "pause")
                      return (
                        <span
                          key={idx}
