...
import { formatNormLabel } from "./OrgNormsPresetSelector";
...
function renderLabel(option: string, category: keyof OrgNorms) {
  const match =
    ORG_NORM_PRESETS?.[category]?.find((preset: OrgNormPresetOption) => preset.value === option);
...
function renderNormEntries(
  ...
  const toEntry = (option: string): { label: string; description?: string | null } => {
    if (option === "other") {
      return {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Shared expectations for decision-making, communication, and safety. Visible to
          all members.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {!hasAnyNorm ? (
          <p className="text-sm text-muted-foreground">
            No norms have been published yet. Owners and admins can add them from
            Settings.
          </p>
        ) : (
                  ...
                  {entries.length === 0 ? (
                    ...
                    <div className="mt-1 space-y-2">
                      {entries.map((entry, idx) => (
                        <div key={`${category}-${entry.label}-${idx}`} className="text-sm">
