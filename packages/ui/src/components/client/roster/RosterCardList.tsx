...
import { useCourseLabelLookup } from "@workspace/ui/hooks/useCourseLabelLookup";
import {
...
export function RosterCardList({
  ...
  const [expandedCertRows, setExpandedCertRows] = React.useState<Record<string, boolean>>({});
    ...
    <div className="grid gap-4 mt-3">
      {rows.map((r) => {
        ...
        const skills = Array.isArray(r.skills)
          ? r.skills.filter((skill) => typeof skill === "string" && skill.trim().length > 0)
          : [];
            ...
            <CardContent className="flex flex-col gap-6 text-sm">
              ...
              <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <InfoTile
                  ...
                  value={
                    certSummary.total
                      ? `${certSummary.total} ${certSummary.total === 1
                        ? "cert"
                        : "certs"
                      }`
                      : "—"
              ...
              <section className="space-y-3">
                {certSummary.total > 0 ? (
                  showCertCollapsible ? (
                    <Collapsible
                      open={isCertExpanded}
                      onOpenChange={(open) => handleCertificationToggle(r.id, open)}
                      className="space-y-3"
                    >
...
function formatCertificationSummary(summary: CertificationSummary): string {
  ...
  return parts.length > 0 ? parts.join(" • ") : "No certifications tracked yet.";
}
