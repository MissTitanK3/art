...
export function PublicImpactSummary({ regionId, className }: PublicImpactSummaryProps) {
    ...
    if (state.status === "hidden") {
      return (
        <p className="text-sm text-muted-foreground">
          Impact metrics are withheld until more verified dispatches are available. Check back soon.
        </p>
      );
    }
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs uppercase tracking-wide text-muted-foreground">Community lift (30d)</p>
        </div>
        <div className="flex items-baseline gap-4">
          <div>
            <p className="text-sm text-muted-foreground">Volunteer hours</p>
            <p className="text-3xl font-semibold">{payload.hours.toLocaleString()}</p>
          </div>
          <div>
  ...
  return (
    <Card className={cn("shadow-sm", className)}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-base font-semibold">Regional Impact</CardTitle>
