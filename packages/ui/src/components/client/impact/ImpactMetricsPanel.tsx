...
const RISK_LEVEL_DETAILS: { value: ImpactRiskLevel; label: string; description: string; tone: string }[] = [
  { value: "unknown", label: "Unknown", description: "Risk not yet assessed.", tone: "bg-muted text-foreground" },
  { value: "low", label: "Low", description: "Routine follow-up, minimal escalation expected.", tone: "bg-emerald-100 text-emerald-900" },
  { value: "medium", label: "Medium", description: "Heightened monitoring. Field team should check-in frequently.", tone: "bg-amber-100 text-amber-900" },
  { value: "high", label: "High", description: "Elevated threat to neighbors or volunteers. Dispatchers actively coordinating response.", tone: "bg-orange-100 text-orange-900" },
...
export function ImpactMetricsPanel({ dispatchId, status, initialMetrics }: Props) {
  ...
  const fetchMetrics = React.useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Unable to load impact metrics");
    } finally {
      setLoading(false);
    }
  }, [dispatchId]);
  ...
  const saveMetrics = async (patch: Partial<DispatchImpactMetrics>, field: string) => {
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Unable to update impact metrics");
      setError(err instanceof Error ? err.message : "Unable to update impact metrics");
    } finally {
      setSavingField(null);
    }
  };

  const currentRisk = RISK_LEVEL_DETAILS.find((detail) => detail.value === (draft.risk_level as ImpactRiskLevel));
    ...
    <TooltipProvider>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">People helped, resources moved, and perceived risk.</p>
        </div>
          ...
          >
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs text-xs">
                Count unique individuals reached with direct support (legal, mutual aid, accompaniment, etc.).
              </TooltipContent>
            </Tooltip>
          </label>
          <Input
            ...
            onChange={(event) => setDraft((prev) => ({ ...prev, people_served: Number(event.target.value || 0) }))}
            onBlur={() => {
              if (disabled) return;
              if (metrics?.people_served === draft.people_served) return;
              saveMetrics({ people_served: Math.max(0, draft.people_served) }, "people_served");
            }}
          />
        </div>
        <div>
          ...
          >
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs text-xs">
                Track discrete supply units delivered (meal kits, gallons of water, hygiene packs, etc.).
              </TooltipContent>
            </Tooltip>
          </label>
          <Input
            ...
            disabled={disabled || loading}
            onChange={(event) =>
              setDraft((prev) => ({ ...prev, resources_distributed: Number(event.target.value || 0) }))
            }
            onBlur={() => {
              if (disabled) return;
              if (metrics?.resources_distributed === draft.resources_distributed) return;
              saveMetrics({ resources_distributed: Math.max(0, draft.resources_distributed) }, "resources_distributed");
            }}
          />
        </div>
      </div>
      ...
      <div className="mt-4 space-y-2">
        ...
        >
          ...
          <Tooltip>
            ...
            <TooltipContent className="max-w-xs text-xs">
              Set how urgent or dangerous this dispatch remains after verification.
            </TooltipContent>
          </Tooltip>
        </label>
        <Select
        ...
        {currentRisk ? (
          <div
            className={`rounded-md px-3 py-2 text-xs ${currentRisk.tone}`}
          >
