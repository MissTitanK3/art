...
export type SignInSubmit = (email: string, password: string) => Promise<void> | void;
...
export function SignInCard({ onSubmit, pendingText = "Signing in…", title = "Sign in", description = "Use your email and password to sign in.", footer }: SignInCardProps) {
    ...
    <Card className="mx-auto mt-12 w-full max-w-md border-border/60 bg-background/95 backdrop-blur">
      ...
      <CardContent>
        <form className="grid gap-4" onSubmit={handleSubmit}>
          ...
          <Button type="submit" disabled={pending}>{pending ? pendingText : "Sign in"}</Button>
        </form>
      </CardContent>
