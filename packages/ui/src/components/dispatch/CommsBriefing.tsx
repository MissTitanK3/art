...
const SECTION_CONFIG: Array<{
  ...
  description: string;
}> = [
    {
      ...
      description: "Give a one-minute story and a quick checklist for anyone walking in.",
    },
    {
      key: "comms_plan",
      label: "Comms Plan",
      ...
      {MARKDOWN_KEY.map((item) => (
        <Popover key={item.label}>
          ...
          >
            ...
            <div>
              <p className="text-[12px] font-semibold">What teammates will see</p>
              <MarkdownPreview
                className="prose-xs"
                source={item.preview}
                emptyText=""
...
export function CommsBriefing({ briefing, onSave, className }: Props) {
  ...
  const persisted = React.useMemo(() => deriveBriefingState(briefing), [briefing]);
