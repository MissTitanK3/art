...
type Props = {
  ...
  onCreateAlert?: (input: { direction: string; description: string }) => Promise<string> | string | void;
  onUpdateAlert?: (id: string, patch: Partial<{ direction: string; description: string }>) => Promise<void> | void;
...
export function CommsAlertsCard({ alerts: extAlerts, onCreateAlert, onUpdateAlert, onDeleteAlert, storageKey = 'comms-alerts:default' }: Props) {
  ...
  const addAlert = async () => {
    if (useExternal && onCreateAlert) {
      ...
      setLocalAlerts((prev) => [...prev, { id: crypto.randomUUID(), direction: '', description: '' }]);
    }
  };
      ...
      <CardContent className="grid gap-4 text-sm">
        <div className="grid gap-2">
          ...
          <div className="grid gap-3">
            ...
            {(useExternal ? (extAlerts?.length ?? 0) : localAlerts.length) === 0 && (
              <p className="text-muted-foreground text-xs">No alerts yet. Click “Add alert” or “Reset to examples” to get started.</p>
            )}
          </div>
        </div>
        ...
        <div className="grid gap-2">
          ...
          <div className="grid gap-1">
            ...
            <ul className="list-disc space-y-1 pl-4 text-muted-foreground">
              <li>Monitor Channel 16 when not actively talking on another channel.</li>
              <li>Hail on Channel 16, then switch immediately to a working channel to converse.</li>
              <li>Be clear and concise: speak slowly, use standard phrases, keep messages brief.</li>
              <li>Use proper terminology: “Roger”/“Copy that” to acknowledge; “Over” to yield; “Out” to end.</li>
              <li>Identify your vessel or party at the start and end of transmissions.</li>
              <li>Listen first: ensure the channel is clear before transmitting.</li>
              <li>Perform radio checks on a working channel with a specific station (e.g., a marina).</li>
            </ul>
          </div>
          <div className="grid gap-1">
            <p className="font-semibold">Don’ts</p>
            <ul className="list-disc space-y-1 pl-4 text-muted-foreground">
              <li>Don’t use Channel 16 for casual chat; it’s for distress and hailing only.</li>
              <li>Don’t use profanity; transmissions are public.</li>
              <li>Don’t monopolize channels; keep transmissions short so others can use them.</li>
              <li>Don’t interrupt or talk over others, especially during distress traffic.</li>
              <li>Don’t say “Over and out”; they are contradictory—use one or the other appropriately.</li>
              <li>Don’t transmit false distress calls; it’s a serious offense.</li>
            </ul>
          </div>
        </div>
      </CardContent>
