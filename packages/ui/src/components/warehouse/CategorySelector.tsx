...
export function CategorySelector({ field, availableCategories }: CategorySelectorProps) {
    const isCustomCategory = field.value && !availableCategories.includes(field.value.toLowerCase());
    const selectValue = isCustomCategory ? "__custom__" : (field.value?.toLowerCase() || "");
            ...
            {(field.value === "" || isCustomCategory) && (
                <FormControl>
                    <Input
                        ...
                        value={field.value || ""}
                        onChange={(e) => {
                            const normalized = e.target.value.toLowerCase().trim().replace(/\s+/g, '-');
