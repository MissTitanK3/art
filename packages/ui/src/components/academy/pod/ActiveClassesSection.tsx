...
import { formatDurationLabel, formatNextSessionLabel, modalityLabels, instructorTypeLabels } from './utils'
...
export function ActiveClassesSection({ classes, onScheduleClass }: ActiveClassesSectionProps) {
  const handleScheduleClass = onScheduleClass ?? (() => {})

  const activeClasses = React.useMemo(() => {
    const filtered = classes.filter((trainingClass) => trainingClass.status !== 'completed')
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Cohorts currently being drafted, staffed, or scheduled.</p>
        </div>
              ...
              <Card key={trainingClass.id} className="flex flex-col border border-border/70 shadow-none">
                ...
                <CardContent className="flex flex-1 flex-col gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-medium">{nextSessionLabel ?? 'Not scheduled'}</p>
                  </div>
                  <div className="mt-auto flex justify-end">
                    <Button type="button" size="sm" variant="outline" onClick={() => handleScheduleClass(trainingClass.id)}>
                      Manage class
                    </Button>
                  </div>
                </CardContent>
