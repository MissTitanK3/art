...
function LengthMeter({ text }: { text: string }) {
  ...
  const barColor = severity === 'danger' ? 'bg-red-500' : severity === 'warn' ? 'bg-amber-500' : 'bg-emerald-500';
  const textColor = severity === 'danger' ? 'text-red-600' : severity === 'warn' ? 'text-amber-600' : 'text-emerald-600';
  return (
    <div className="mt-2">
      ...
      <div className={`mt-1 text-xs ${textColor}`}>
        Words: {wc} · Chars: {cc} · Est: {est}s — {severity === 'ok' ? 'Good length' : severity === 'warn' ? 'Consider splitting' : 'Too long—split this line'}
      </div>
    </div>
  );
}
...
export default function TeleprompterScriptBuilder({ builtinScripts, builtinMeta, storageNamespace = 'teleprompter.builder' }: TeleprompterScriptBuilderProps) {
  ...
  const [saved, setSaved] = useLocalStorage<{ id: string; name: string; lines: BuilderLine[] }[]>(
    ns('saved'),
    []
  );
  const [placeholderMap, setPlaceholderMap] = React.useState<Record<string, string>>({});
  ...
  const addLine = () => {
    ...
    setLines((prev) => [...prev, { id: uid(), text: currentText.trim(), cues: currentCues }]);
    setCurrentText('');
    setCurrentCues([]);
  };

  const toggleCue = (cue: string) => {
    setCurrentCues((prev) => (prev.includes(cue) ? prev.filter((c) => c !== cue) : [...prev, cue]));
  };
  ...
  const addCustomCue = () => {
    ...
    setCurrentCues((prev) => (prev.includes(normalized) ? prev : [...prev, normalized]));
    setCustomCue('');
  };
  ...
  const [dragOverPos, setDragOverPos] = React.useState<'above' | 'below' | null>(null);
  ...
  const compiledLines = lines.map((l) => (l.cues.length ? `${l.text} ${l.cues.join(' ')}` : l.text));

  const moveLineAtPosition = (from: number, index: number, pos: 'above' | 'below') => {
  ...
  const tsExport = `// Add this to TELEPROMPTER_SCRIPTS and TELEPROMPTER_SCRIPT_META\n// id suggestion: '${name.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '')}'\n[\n${compiledLines
    .map((l) => `  ${JSON.stringify(l)}`)
    .join(',\n')}\n].join("\\n\\n")`;

  const suggestedId = React.useMemo(() => name.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, ''), [name]);
  ...
  const filledPreview = React.useMemo(() => {
    ...
    const replaced = Object.entries(placeholderMap).reduce((acc, [k, v]) => acc.replaceAll(`[${k}]`, v || `[${k}]`), all);
    return replaced;
  ...
  const QUICK_TEMPLATES = React.useMemo(() => {
    ...
    const picks = candidates
      ...
      .filter((x) => !!x.meta)
      .slice(0, 3) as Array<{ id: string; meta: { id: string; label: string } }>;
    return picks.map((p) => ({ id: p.meta.id, title: p.meta.label, blurb: 'Quick-start template' }));
  }, [builtinMeta]);
    ...
    <div className="space-y-8">
      <section className="rounded-lg border bg-card p-4 shadow-sm">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Write one teleprompter line at a time. Keep each line focused. Use cues to guide delivery. If the meter turns
          amber/red, split the line.
        </p>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const asLines: BuilderLine[] = chunks.map((chunk) => {
                    const cues = Array.from(chunk.matchAll(/\[(.*?)\]/g)).map((m) => m[0]).filter(Boolean);
                    const text = chunk.replace(/\s*\[(.*?)\]/g, '').trim();
                    return { id: uid(), text, cues };
                  });
                  setLines(asLines);
                  setName(builtinMeta.find((m) => m.id === tpl.id)?.label || tpl.title);
                }}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                setLines(asLines);
                if (!name || name === 'My Script') setName(builtinMeta.find((m) => m.id === presetId)?.label || 'Preset');
              }}
      ...
      <section className="rounded-lg border bg-card p-4 shadow-sm">
        ...
        <p className="mt-1 text-xs text-muted-foreground md:hidden">Tip: On phones, use the arrow buttons to reorder; drag-and-drop may not be available on touch.</p>
        {!mounted ? (
          ...
          <ol className="mt-3 space-y-3" role="list">
            {lines.map((l, idx) => (
              ...
              >
                ...
                <div className="mt-1 select-none text-xs text-muted-foreground">{idx + 1}</div>
                <div className="flex-1">
                  <div className="whitespace-pre-wrap text-sm">{l.text}</div>
                  {l.cues.length > 0 && (
                    <div className="mt-1 flex flex-wrap gap-1">
                      {l.cues.map((c) => (
                        <span key={c} className="rounded-full border px-2 py-1 text-xs sm:py-0.5 sm:text-[10px]">
                          {c}
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex items-center gap-2 flex-wrap justify-evenly sm:justify-start">
                  ...
                  <button
                    type="button"
                    onClick={() => idx < lines.length - 1 && moveLine(idx, idx + 1)}
                  ...
                  <button
                    type="button"
                    onClick={() => { if (idx !== 0) { moveLine(idx, 0); setLiveMessage(`Moved to position 1 of ${lines.length}.`); } }}
                  ...
                  <button
                    type="button"
                    onClick={() => { if (idx !== lines.length - 1) { moveLine(idx, lines.length - 1); setLiveMessage(`Moved to position ${lines.length} of ${lines.length}.`); } }}
      ...
      <section className="rounded-lg border bg-card p-4 shadow-sm">
        ...
        {mounted && <div className="sr-only" aria-live="polite">{liveMessage}</div>}
        <p className="mt-1 text-sm text-muted-foreground">
          This is how your teleprompter script will render (each line separated by an empty line).
        </p>
        ...
        {placeholdersInUse.length > 0 && (
          <div className="mt-4 grid gap-2 rounded-md border p-3">
            <div className="text-sm font-medium">Inline placeholder preview</div>
              ...
              {placeholdersInUse.map((k) => (
                <div key={k} className="grid gap-1 min-w-0">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setPlaceholderMap((m) => ({ ...m, [k]: e.target.value }))}
            ...
            <div>
              <div className="mt-2 text-xs text-muted-foreground">Filled preview</div>
              <pre className="mt-1 w-full max-h-60 overflow-auto rounded-md border bg-background p-3 text-sm whitespace-pre-wrap break-words">{filledPreview}</pre>
            </div>
          </div>
        )}
      </section>
      ...
      <section className="rounded-lg border bg-card p-4 shadow-sm">
        ...
        <div className="mt-3 flex flex-col sm:flex-row sm:flex-wrap gap-2">
          <button
            type="button"
            onClick={() => { if (typeof navigator !== 'undefined' && navigator.clipboard) navigator.clipboard.writeText(jsonExport); }}
          ...
          <button
            type="button"
            onClick={() => download(`${name.replace(/\s+/g, '_')}.json`, jsonExport)}
            className="w-full sm:w-auto rounded-md border px-3 py-2 text-base sm:text-sm hover:bg-accent"
          >
            Download JSON
          </button>
          <button
            type="button"
            onClick={() => { if (typeof navigator !== 'undefined' && navigator.clipboard) navigator.clipboard.writeText(tsExport); }}
            className="w-full sm:w-auto rounded-md border px-3 py-2 text-base sm:text-sm hover:bg-accent"
          >
            Copy TS snippet
          </button>
        ...
        <div className="mt-4 grid gap-3 rounded-md border p-3 overflow-hidden">
          ...
          <div className="text-xs text-muted-foreground max-w-prose break-words">
            ...
            <code className="mx-1 break-words">{'{'} name, lines: string[] {'}'}</code>
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              try {
                ...
                if (!Array.isArray(importedLines)) throw new Error('Invalid JSON: lines must be an array');
                const toLines: BuilderLine[] = importedLines.map((item: any) => {
                  if (typeof item === 'string') {
                    const cues = Array.from(item.matchAll(/\[(.*?)\]/g)).map((m: any) => m[0]).filter(Boolean);
                    const textOnly = item.replace(/\s*\[(.*?)\]/g, '').trim();
                    return { id: uid(), text: textOnly, cues };
                  }
                  if (item && typeof item.text === 'string') {
                    const t = item.text as string;
                    const cues = Array.isArray(item.cues) ? item.cues as string[] : Array.from(t.matchAll(/\[(.*?)\]/g)).map((m: any) => m[0]).filter(Boolean);
                    return { id: uid(), text: t.replace(/\s*\[(.*?)\]/g, '').trim(), cues };
                  }
          ...
          <div className="grid gap-2 w-full">
            ...
            />
            <div className="text-xs text-muted-foreground break-words">Tip: paste JSON and click elsewhere to load it.</div>
          </div>
        </div>
        <div className="mt-4 grid gap-3 rounded-md border p-3">
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2">
            <button type="button" onClick={() => { if (typeof navigator !== 'undefined' && navigator.clipboard) navigator.clipboard.writeText(coreScriptsSnippet); }} className="w-full sm:w-auto rounded-md border px-3 py-2 text-base sm:text-sm hover:bg-accent">Copy core entries</button>
            <button type="button" onClick={() => download(`${(coreId || suggestedId || 'custom_script')}.core.ts.txt`, coreScriptsSnippet)} className="w-full sm:w-auto rounded-md border px-3 py-2 text-base sm:text-sm hover:bg-accent">Download entries</button>
          </div>
        </div>
        {saved.length > 0 && (
          <div className="mt-4">
            ...
            <ul className="mt-2 space-y-2">
              {saved.map((s) => (
                <li key={s.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 rounded-md border bg-background p-2 text-sm">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                    ...
                    <button
                      type="button"
                      onClick={() => download(`${s.name.replace(/\s+/g, '_')}.json`, JSON.stringify({ name: s.name, lines: s.lines.map((l) => (l.cues.length ? `${l.text} ${l.cues.join(' ')}` : l.text)) }, null, 2))}
                      className="w-full sm:w-auto rounded-md border px-3 py-2 text-sm sm:px-2 sm:py-1 sm:text-xs hover:bg-accent"
                    >
                      Download
                    </button>
      ...
      <section className="rounded-lg border bg-card p-4 shadow-sm">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-muted-foreground">
          <li>Lead with “Who we are”: “We are with [ORG] and our names are [NAMES].”</li>
          <li>One idea per line. Use cues like [pause], [look up], [breathe] to pace.</li>
          <li>Keep sensitive info out of on-air comms; use placeholders like [LOCATION] or route via dispatch.</li>
          <li>Prefer verbs: observe, document, escalate. Avoid debating in the field.</li>
          <li>Use the meter. If it goes amber/red, split the line for smoother delivery.</li>
        </ul>
      </section>
    </div>
  );
