...
export function NotificationPrefsForm({ loading, saving, channels, globalOptOut, muted, onToggleReceive, onToggleChannel, onSave }: NotificationPrefsFormProps) {
  return (
    <FormSectionCard
      title="Notifications"
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <Switch checked={!globalOptOut} onCheckedChange={(v) => onToggleReceive(v)} disabled={loading} />
      </div>
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {channels.map((c) => (
            <label key={c} className="flex items-center gap-2">
              <Checkbox checked={!!muted[c]} onCheckedChange={() => onToggleChannel(c)} disabled={!!loading || globalOptOut} />
