...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        // width is caller-controlled: use w-full or a fixed width on usage sites
        ...
        "aria-invalid:border-destructive aria-invalid:focus-visible:ring-destructive/20 dark:aria-invalid:focus-visible:ring-destructive/40",
        "dark:bg-input/30 dark:hover:bg-input/50",
        "data-[size=default]:h-9 data-[size=sm]:h-8",
        "data-[placeholder]:text-muted-foreground",
        "[&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
        // keep value inline and ellipsized
...
function SelectContent({
  className,
  children,
  position = "popper", // ← default globally to popper
  sideOffset = 4,
  ...props
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          // Ensure content appears above drawers/overlays (drawer uses z-[70])
          "relative z-[80] min-w-[8rem] overflow-x-hidden overflow-y-auto rounded-md border bg-popover text-popover-foreground shadow-md",
          "data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95",
          "data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95",
          "data-[side=bottom]:slide-in-from-top-2 data-[side=top]:slide-in-from-bottom-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2",
          // Use the available height and correct transform-origin vars
          "max-h-[var(--radix-select-content-available-height)] origin-[var(--radix-select-content-transform-origin)]",
          position === "popper" &&
            // Nudge out from the anchor slightly to avoid overlap
      ...
      >
        ...
        <SelectPrimitive.Viewport
          className={cn(
            "p-1",
            position === "popper" &&
              // Lock viewport to trigger size to prevent CLS
