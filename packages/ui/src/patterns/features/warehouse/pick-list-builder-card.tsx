    ...
    <div className={cn("flex items-center gap-1", className)}>
      ...
      <div className="relative flex items-center justify-center w-16 h-9">
        {isSyncing ? (
          ...
          <span className={cn("font-semibold text-lg", valueClassName)}>
            {localValue}
          </span>
        )}
      </div>
      <Button
        ...
        onClick={() => handleUpdate(localValue + 1)}
        disabled={
          disabled || (max !== undefined && localValue >= max) || isSyncing
        }
        className="h-9 w-9 p-0"
      >
...
export function PickListBuilderCard({
  ...
  const hasActiveFilters =
    nameFilter.trim() !== "" || warehouseFilter !== "all";
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Dispatchers build a pull list, stewards confirm to reduce stock and
          log outflow.
        </CardDescription>
      </CardHeader>
              ...
              <div className="flex flex-wrap gap-2">
                {paginatedInventory.map((entry) => {
                  ...
                  const pick = pickList.find(
                    (item) => item.inventoryId === entry.id
                  );
                  const confirmedPick = confirmedPickLists?.find(
                    (item) => item.inventoryId === entry.id
                  );
                  const allocatedQty =
                    (pick?.quantity ?? 0) + (confirmedPick?.quantity ?? 0);
          ...
          {pickList.length === 0 ? (
            ...
            <div className="space-y-2 flex flex-col md:flex-row gap-2 wrap">
              {pickList.map((item) => {
                ...
                const location =
                  inventoryRow &&
                  resolveInventoryLocation(inventoryRow, warehouses);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center justify-center gap-2 w-full">
                      <hr className="w-full" />
                      <p className="text-sm">Quantity Request:</p>
                      <DebouncedQuantitySelector
                        value={item.quantity}
