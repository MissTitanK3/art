...
import { CatalogQuickAdd } from "./catalog-quick-add";
import {
...
export function InventoryIntakeForm({
  ...
  if (savedWarehouses.length === 0) {
    return (
      <Alert>
        ...
        <AlertDescription>
          Use the builder to create your first warehouse to unlock intake, pick
          lists, and cycle counts.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <Form {...intakeForm}>
      <form
        className="space-y-4"
        onSubmit={intakeForm.handleSubmit(handleIntakeSubmit)}
      >
        ...
        <Tabs value={intakeTab} onValueChange={handleTabsValueChange}>
          <TabsList className="w-auto min-h-[44px]">
            <TabsTrigger value="existing" className="min-h-[40px]">
              Catalog
            </TabsTrigger>
            <TabsTrigger value="new" className="min-h-[40px]">
              Add new SKU
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="new" className="mt-4">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription className="text-xs sm:text-sm text-muted-foreground">
                      Use short, dash-separated labels. Auto-generate if you
                      need a quick slug.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                  ...
                  <FormItem>
                    ...
                    <FormControl className="w-full">
                      <Input
                        ...
                        placeholder="Lot number, condition note…"
                        {...field}
                      />
                    </FormControl>
                    <FormDescription className="text-xs sm:text-sm">
                      Only in movement logs.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <div className="flex flex-col sm:flex-row sm:justify-end gap-3">
              ...
              <Button
                type="submit"
                disabled={
                  !intakeForm.formState.isValid || savedWarehouses.length === 0
                }
                className="w-full sm:w-auto min-h-[44px]"
              >
                Log Intake
