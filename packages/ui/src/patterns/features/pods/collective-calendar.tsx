...
import { addHours, isSameDay, isWithinInterval, parseISO } from "date-fns";
import { BadgeInfo, Layers } from "lucide-react";
...
import {
  ...
  needsRemaining,
} from "./collective-calendar-shared";
import { CollectiveCalendarShiftDetailsSheet } from "./collective-calendar-shift-details-sheet";
import { CollectiveCalendarFilters } from "./collective-calendar-filters";
import { CollectiveCalendarView } from "./collective-calendar-view";
import { CollectiveCalendarShiftForm } from "./collective-calendar-shift-form";
      ...
      {canManageShifts && (
        <CollectiveCalendarShiftForm
          ...
          defaultPodId={
            selectedPod !== "all"
              ? selectedPod
              : (memberOrganizations.find((o) => (o.pods?.length ?? 0) > 0)
                  ?.pods?.[0]?.id ?? pods[0]?.id)
          }
