...
import { CollectiveCalendarShiftCard } from "./collective-calendar-shift-card";
    ...
    sortedSelectedDayShifts.forEach((shift) => {
      ...
      hours.forEach((block) => {
        if (shiftStart < block.hourEnd && shiftEnd > block.hourStart) {
          const isStart =
            shiftStart >= block.hourStart && shiftStart < block.hourEnd;
          ...
          <div className="rounded-md border p-3">
            ...
            <div className="mt-2 grid grid-cols-7 gap-1">
              {days.map((day) => {
                ...
                const showCountOnly =
                  shiftCount === 0 ? null : String(shiftCount);
                const countLength = showCountOnly
                  ? Math.min(showCountOnly.length, 3)
                  : 1;
