...
export function CollectiveCalendarShiftForm({
  ...
  useEffect(() => {
    ...
    const endSource =
      initialShift?.end ??
      draftEnd ??
      (draftStart ? addHours(new Date(draftStart), 1).toISOString() : null);

    const initialVisibilityScope =
      initialShift?.visibilityScope ??
      mapVisibilityToScope(initialShift?.visibility ?? "public");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-card text-card-foreground max-h-[85vh] overflow-y-auto p-4 sm:max-w-2xl sm:p-6 z-[1201]">
        <DialogHeader>
          <DialogTitle>{initialShift ? "Edit shift" : "New shift"}</DialogTitle>
          <DialogDescription>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="grid gap-1">
              ...
              <VisibilitySelector
                ...
                onChange={(scope) =>
                  setForm((prev) => ({ ...prev, visibilityScope: scope }))
                }
                disabled={submitting}
              />
