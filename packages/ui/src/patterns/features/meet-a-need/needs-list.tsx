  ...
  if (!needs?.length) {
    return (
      <p className="text-sm text-muted-foreground">
        No needs yet. Be the first to post one.
      </p>
    );
  }
  return (
      ...
      {needs.map((n) => (
        <Card key={n.id}>
          ...
          <CardContent className="space-y-3">
            ...
            {Array.isArray((n as any).location?.media) &&
            (n as any).location.media.length > 0 ? (
                ...
                <div className="flex flex-col items-start gap-1">
                  ...
                  {canManageStatus ? (
                    <Select
                      value={n.status}
                      onValueChange={(v) =>
                        onUpdateStatus(n.id, v as NeedStatus)
                      }
                    >
                ...
                <div className="flex flex-col items-start gap-1">
                  ...
                  {canManageNeed ? (
                    <Select
                      value={n.urgency}
                      onValueChange={(v) =>
                        onUpdateNeed(n.id, { urgency: v as NeedUrgency })
                      }
                    >
                      <SelectTrigger size="sm" className="w-full sm:w-[160px]">
                        <SelectValue />
                    ...
                    .map((r, idx) => (
                      ...
                      >
                        ...
                        <div className="mt-1 text-sm">
                          <span className="mr-2 inline-block rounded border px-2 py-0.5 text-xs capitalize">
                            {r.resource_type}
                          </span>
                          {r.notes ? (
      ...
      >
        <DrawerContent className="p-4 max-w-5xl m-auto bg-card text-card-foreground">
          ...
          <DrawerFooter className="gap-2 sm:flex-row sm:justify-end">
            ...
            <Button variant="secondary" onClick={() => setViewerUrl(null)}>
              Close
            </Button>
          </DrawerFooter>
        </DrawerContent>
  ...
  return (
    <div className="border rounded p-3 space-y-2">
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          <label className="text-xs" htmlFor="offer-resource">
            Resource
          </label>
          <Select
            value={resourceType}
            onValueChange={(v) =>
              setResourceType(v as OfferPayload["resource_type"])
            }
          >
            <SelectTrigger id="offer-resource">
        ...
        <div className="space-y-1">
          <label className="text-xs" htmlFor="offer-notes">
            Notes
          </label>
          <Input
            id="offer-notes"
