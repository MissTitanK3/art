...
export function CommsOperatorList({
  operators,
  ...
  return (
    <div className="space-y-2 text-sm">
      {operators.length === 0 ? (
        ...
        operators.map((op) => {
          const interval =
            op.check_in_interval_minutes ?? defaultCheckInMinutes;
          return (
              ...
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    {op.station_name ?? op.station_type ?? "—"}
                    {op.frequency ? ` · ${op.frequency}` : ""}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <CheckInTimerBadge
                    lastCheckIn={op.last_check_in ?? undefined}
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => onCheckIn?.(op.id)}
                    aria-label={`Check in ${op.callsign}`}
                  >
                    Check In
                  </Button>
                </div>
