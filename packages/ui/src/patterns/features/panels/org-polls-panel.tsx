...
import type { OrgPoll } from "@workspace/ui/patterns/features/orgs/types";
...
const resolveClosingDate = (
  preset: (typeof closingPresets)[number]["value"]
) => {
...
export function OrgPollsPanel({
  ...
  const [closingPreset, setClosingPreset] =
    useState<(typeof closingPresets)[number]["value"]>("72h");
  ...
  const stats = useMemo(() => {
    const open = polls.filter(
      (poll) => getEffectiveStatus(poll) === "open"
    ).length;
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Decision snapshots that stay out of Signal churn. Titles and votes
            only.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Keep sensitive context out of the poll. Use secure channels for
            details and record in org docs.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
                        ...
                        setOptions((prev) =>
                          prev.map((opt) =>
                            opt.id === option.id
                              ? {
                                  ...opt,
                                  emoji: value === EMOJI_NONE ? null : value,
                                }
                              : opt
                          )
                        )
                ...
                onClick={() =>
                  setOptions((prev) => [
                    ...prev,
                    {
                      ...
                      emoji:
                        EMOJI_CHOICES[prev.length % EMOJI_CHOICES.length] ??
                        "🆕",
                    },
                  ])
                }
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setClosingPreset(
                  value as (typeof closingPresets)[number]["value"]
                )
              }
            >
          ...
          <div className="flex items-center justify-between gap-3 rounded-md border bg-muted/30 p-3">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Let voters choose more than one option. Disable to enforce a
                single choice.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">
                {allowMultiple ? "Multi-vote" : "Single vote"}
              </span>
              <Switch
                checked={allowMultiple}
                onCheckedChange={(checked) =>
                  setAllowMultiple(Boolean(checked))
                }
              />
            </div>
          </div>
      ...
      {polls.length === 0 ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Use polls to capture lightweight decisions. The reminder about
              keeping PII out of polls is attached to every poll automatically.
            </CardDescription>
          </CardHeader>
        </Card>
      ) : (
              ...
              <Card key={poll.id}>
                <CardHeader className="space-y-1">
                  ...
                  <CardDescription className="flex flex-wrap items-center gap-3 text-xs">
                    <span>
                      Votes are recorded here; coordinate context in a secure
                      channel.
                    </span>
                    {poll.createdAt && (
