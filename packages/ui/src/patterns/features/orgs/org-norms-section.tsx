...
import { OrgNormsOtherInput } from "@workspace/ui/patterns/features/orgs/org-norms-other-input";
import {
  OrgNormsPresetSelector,
  formatNormLabel,
} from "@workspace/ui/patterns/features/orgs/org-norms-preset-selector";
...
export function OrgNormsSection({
  ...
  const handleOtherChange = (other: string) => {
    ...
    if (allowMultiple && Array.isArray(currentType)) {
      const nextType = Array.from(
        new Set([...currentType.filter((t) => t !== "other"), "other"])
      );
      const nextValue: OrgNormsMultiCategory = {
        type: nextType,
        other,
    ...
    <section className="space-y-3 rounded-lg border p-3 m-2 h-full">
      <div className="space-y-1">
        <h4 className="text-sm font-medium">
          {title ?? formatNormLabel(category)}
        </h4>
        {description && (
