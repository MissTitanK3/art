...
import InstructorForm, {
  type InstructorFormHandle,
} from "@workspace/ui/patterns/features/pod/instructor-form";
import type {
  ...
  return (
    <section className="space-y-4 m-auto max-w-7xl">
      ...
      <Sheet open={isManageSheetOpen} onOpenChange={setIsManageSheetOpen}>
        <SheetContent
          side="right"
          className="max-w-xl bg-card text-card-foreground p-4"
        >
          <SheetHeader>
            <SheetTitle>
              Manage{" "}
              {selectedInstructor ? selectedInstructor.name : "instructor"}
            </SheetTitle>
            <SheetDescription>
          ...
          <div className="space-y-4">
            {selectedInstructor ? (
              ...
              <p className="text-sm text-muted-foreground">
                Select an instructor to manage.
              </p>
            )}
          </div>
