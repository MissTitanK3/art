...
export function SessionsBoard({
  ...
  const noopCreate = useCallback((_: AcademyTrainingSessionDraft) => {}, []);
  const noopUpdateStatus = useCallback(
    (_: string, __: AcademyTrainingSession["status"]) => {},
    [],
  );
  const noopUpdateSession = useCallback(
    (_: string, __: Partial<AcademyTrainingSession>) => {},
    [],
  );
  const noopDeleteSession = useCallback((_: string) => {}, []);
  ...
  // but that can silently hide mistakes where the parent intentionally gated
  // handlers. Emit a console warning in non-production builds to make this
  ...
  useEffect(() => {
    try {
      ...
      if (!onCreateSession) missing.push("onCreateSession");
      if (!onUpdateSessionStatus) missing.push("onUpdateSessionStatus");
      if (!onUpdateSession) missing.push("onUpdateSession");
      if (!onDeleteSession) missing.push("onDeleteSession");
      if (missing.length > 0) {
        console.warn(
      ...
      // Do not throw in the UI - best-effort warning only.
    }
  }, [
    onCreateSession,
    onUpdateSessionStatus,
  ...
  const [newSessionModality, setNewSessionModality] =
    useState<AcademyTrainingSession["modality"]>("online");
  const [newSessionLocation, setNewSessionLocation] = useState("");
  const [newSessionSeats, setNewSessionSeats] = useState("");
  const [newSessionStatus, setNewSessionStatus] =
    useState<AcademyTrainingSessionDraft["status"]>("scheduled");
  const [newSessionTopic, setNewSessionTopic] = useState("");
  const [createFormError, setCreateFormError] = useState<string | null>(null);
  const [isManageDrawerOpen, setIsManageDrawerOpen] = useState(false);
  const [managedSessionId, setManagedSessionId] = useState<string | null>(null);
  const [manageSessionTitle, setManageSessionTitle] = useState("");
  const [manageSessionStart, setManageSessionStart] = useState("");
  const [manageSessionEnd, setManageSessionEnd] = useState("");
  const [manageSessionModality, setManageSessionModality] =
    useState<AcademyTrainingSession["modality"]>("online");
  const [manageSessionStatus, setManageSessionStatus] =
    useState<AcademyTrainingSession["status"]>("scheduled");
  const [manageSessionLocation, setManageSessionLocation] = useState("");
  const [manageSessionMeetingUrl, setManageSessionMeetingUrl] = useState("");
  const [manageSessionCapacity, setManageSessionCapacity] = useState("");
  const [manageSessionInstructorName, setManageSessionInstructorName] =
    useState("");
  const [manageSessionInstructorType, setManageSessionInstructorType] =
    useState<AcademyInstructorProfile["type"]>("dispatcher");
  const [manageSessionTimezone, setManageSessionTimezone] = useState("");
  const [manageSessionTopic, setManageSessionTopic] = useState("");
  const [manageSessionParticipants, setManageSessionParticipants] = useState<
    EditableParticipant[]
  >([]);
  const [manageFormError, setManageFormError] = useState<string | null>(null);
  const createParticipantId = useCallback(() => {
  ...
  const waitlistParticipantsCount = useMemo(
    () =>
      manageSessionParticipants.filter(
  ...
  const resolveAbsoluteUrl = useCallback((path: string) => {
  ...
  const copySessionDetailsToClipboard = useCallback(
  ...
  const handleOpenManageSession = useCallback((sessionId: string) => {
    setManagedSessionId(sessionId);
    setManageFormError(null);
    setIsManageDrawerOpen(true);
  }, []);
  const handleManageDrawerChange = useCallback(
    (open: boolean) => {
  ...
  const updateParticipant = useCallback(
    (participantId: string, patch: Partial<EditableParticipant>) => {
    ...
    (
      ...
      if (field === "understanding") {
        updateParticipant(participantId, {
          understanding: value as AcademySessionUnderstandingLevel,
        });
        return;
      }
      if (field === "status") {
        updateParticipant(participantId, {
          status: value as EditableParticipant["status"],
        });
        return;
      }
      updateParticipant(participantId, {
  ...
  const handleSaveManagedSession = useCallback(() => {
    ...
    setManageFormError(null);
    const participantsPayload: AcademyTrainingSessionParticipant[] =
    ...
    handleUpdateSession(managedSessionId, {
      ...
      instructorName:
        manageSessionInstructorName.trim() || managedSession.instructorName,
  ...
  const renderActiveSessionCard = useCallback(
  ...
  const renderArchivedSessionCard = useCallback(
    (session: AcademyTrainingSession) => {
      if (session.status !== "archived") {
        return null;
      }
        ...
        <Card key={session.id} className="border border-border/70 shadow-none">
          <CardContent className="space-y-3 p-4">
            ...
            <div className="flex flex-col gap-1 text-xs text-muted-foreground break-words">
              ...
              {session.relatedTopic ? (
                <span>Topic: {session.relatedTopic}</span>
              ) : null}
            </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              {activeSessionsCount} active sessions coordinating in-person and
              online instruction
              {archivedSessions.length > 0
                ? ` · ${archivedSessions.length} archived`
                : ""}
              . Update status as pods learn.
            </p>
        ...
        {sessionStatusFilter === "all" ? (
          ...
          <div className="space-y-3">
            {filteredSessions.length === 0 ? (
              <p className="rounded-lg border border-dashed border-border/60 p-4 text-sm text-muted-foreground">
                No sessions currently marked as{" "}
                {sessionStatusFilter.replaceAll("_", " ")}.
              </p>
            ) : (
        ...
        <section className="space-y-4">
          <div className="flex flex-col gap-2 lg:flex-row items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Sessions hidden after review. Restore them to the board or
      ...
      <Drawer open={isManageDrawerOpen} onOpenChange={handleManageDrawerChange}>
        <DrawerContent className="bg-card text-card-foreground m-auto h-full w-full max-w-3xl rounded-t-3xl border border-border/70 shadow-xl">
          <DrawerHeader className="sm:text-left">
            ...
            <DrawerDescription>
              Review session details, update scheduling information, and track
              participants.
            </DrawerDescription>
          </DrawerHeader>
            ...
            {managedSession ? (
              <>
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        setManageSessionTitle(event.target.value)
                      }
                ...
                <div className="grid gap-4 md:grid-cols-1">
                  <DateTimePicker
                    label="Starts"
                    value={manageSessionStart}
                    onChange={setManageSessionStart}
                  />
                  <DateTimePicker
                    label="Ends"
                    value={manageSessionEnd}
                    onChange={setManageSessionEnd}
                  />
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="manage-session-instructor-type">
                      Instructor type
                    </Label>
                    <Select
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="manage-session-instructor-name">
                      Instructor name
                    </Label>
                    <Input
                      ...
                      onChange={(event) =>
                        setManageSessionInstructorName(event.target.value)
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        setManageSessionTimezone(event.target.value)
                      }
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        setManageSessionLocation(event.target.value)
                      }
                      placeholder="Dispatch HQ — Training Room"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="manage-session-meeting-link">
                      Meeting link
                    </Label>
                    <Input
                      ...
                      onChange={(event) =>
                        setManageSessionMeetingUrl(event.target.value)
                      }
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        setManageSessionCapacity(event.target.value)
                      }
                      inputMode="numeric"
                      min={1}
                      type="number"
                    />
                    <p className="text-xs text-muted-foreground">
                      {confirmedParticipantsCount} confirmed ·{" "}
                      {waitlistParticipantsCount} waitlist
                    </p>
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        setManageSessionTopic(event.target.value)
                      }
                ...
                <div className="space-y-3">
                  <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Track confirmed seats and waitlist to align with session
                        capacity.
                      </p>
                    </div>
                    <Button
                  ...
                  {manageSessionParticipants.length === 0 ? (
                    <p className="rounded-lg border border-dashed border-border/60 p-3 text-sm text-muted-foreground">
                      No participants yet. Add confirmed attendees or maintain a
                      waitlist.
                    </p>
                  ) : (
                    <div className="space-y-3">
                      {manageSessionParticipants.map((participant) => (
                        ...
                        >
                          <div className="grid gap-3 md:grid-cols-2">
                            ...
                            <div className="space-y-2">
                              <Label
                                htmlFor={`participant-signal-${participant.id}`}
                              >
                                Signal handle
                              </Label>
                              <Input
                          ...
                          <div className="grid gap-3 md:grid-cols-2">
                            <div className="space-y-2">
                              <Label
                                htmlFor={`participant-understanding-${participant.id}`}
                              >
                                Understanding level
                              </Label>
                              <Select
                              ...
                              >
                                <SelectTrigger
                                  id={`participant-understanding-${participant.id}`}
                                >
          ...
          <DrawerFooter className="gap-2 border-t border-border/70 bg-muted/20 sm:flex-row sm:justify-end">
            ...
            <Button
              onClick={handleSaveManagedSession}
      ...
      >
        <DrawerContent className="bg-card text-card-foreground m-auto h-full w-full max-w-2xl rounded-t-3xl border border-border/70 shadow-xl">
          <DrawerHeader className="sm:text-left">
            ...
            <DrawerDescription>
              Plan a focused learning block for individuals or pods—even if it
              is not tied to an existing academy topic.
            </DrawerDescription>
          </DrawerHeader>
          ...
          <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-4 pb-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="new-session-location">
            ...
            <div className="space-y-2">
              <Label htmlFor="new-session-topic">
                Related academy topic (optional)
              </Label>
              <Input
