...
import { CoverageAreaToggleButton } from "./coverage-area-toggle-button.tsx";
...
function certificationMatchesCourse(
  ...
  if (
    cert.display_name &&
    normalizeCourseKey(cert.display_name) === courseKey
  ) {
    return true;
  }
  return false;
...
export function RosterEntryCoverageSection({
  ...
  const handleToggleCoverageArea = useCallback(
    (areaKey: string) => {
      const area = OPERATIONAL_COVERAGE_AREAS.find(
        (entry) => entry.key === areaKey
      );
      if (!area) return;
      const currentCerts = getValues("certs") ?? [];
      const currentStatuses = computeCoverageAreaStatuses(currentCerts);
      const targetStatus = currentStatuses.find(
        (entry) => entry.area.key === areaKey
      );
