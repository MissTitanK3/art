...
function enhanceFlowchart(chart: string, isDark: boolean) {
  ...
  for (const raw of lines) {
    ...
    if (/^(classDef|class |style |linkStyle |click |subgraph|end)\b/.test(line))
      continue;
  ...
  const appendix = [
    ...
    ...yes.map(
      (i) =>
        `linkStyle ${i} stroke:${yesColor},stroke-width:2px,color:${edgeText},font-weight:bold;`,
    ),
    ...no.map(
      (i) =>
        `linkStyle ${i} stroke:${noColor},stroke-width:2px,color:${edgeText},font-weight:bold;`,
    ),
  ];
...
export function Mermaid({ chart, className, theme = "auto" }: MermaidProps) {
  ...
  useEffect(() => {
    ...
    labels.forEach((g) => {
      ...
      const foreign = g.querySelector(
        "foreignObject",
      ) as unknown as SVGForeignObjectElement | null;
      if (foreign) {
