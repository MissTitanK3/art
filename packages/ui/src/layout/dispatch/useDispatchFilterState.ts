...
export function useDispatchFilterState({
  ...
  const resolvedInitialPageSize =
    typeof initialPageSize === 'number' && initialPageSize > 0 ? initialPageSize : defaultPageSize;
  const storageKey = persistKey ?? '__dispatch-filter-state__noop__';

  const defaultPersistedFilters = React.useMemo<PersistedDispatchFilters>(
    () => ({
      query: '',
      status: 'all',
      type: 'all',
  ...
  const [persistedFilters, setPersistedFilters] = useLocalStorage<PersistedDispatchFilters>(
    storageKey,
  ...
  const [pageSize, setPageSize] = React.useState<number>(resolvedInitialPageSize);
