...
export function DispatchListLayout({
  ...
  const [dateRange, setDateRange] = React.useState<{ from?: Date; to?: Date }>(() => ({ from: new Date() }));
  ...
  const tabScopedSubmissions = React.useMemo(() => {
    ...
    if (activeTab === "upcoming") {
      return submissions.filter((s) => s.status !== "archived" && s.date_of_event && new Date(s.date_of_event) >= now);
    }
    // past & archived
    return submissions.filter((s) => s.status === "archived" || (s.date_of_event && new Date(s.date_of_event) < now));
    ...
    const list = tabScopedSubmissions.filter((s) => {
      ...
      if (dateRange.from || dateRange.to) {
        ...
        if (dateRange.from) {
          ...
          const isMidnight = f.getHours() === 0 && f.getMinutes() === 0 && f.getSeconds() === 0 && f.getMilliseconds() === 0;
          fromMs = isMidnight ? f.setHours(0, 0, 0, 0) : f.getTime();
        }
        const toMs = dateRange.to ? new Date(dateRange.to).setHours(23, 59, 59, 999) : undefined;
  ...
  const pastSorted = React.useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      const da = a.date_of_event ? new Date(a.date_of_event).getTime() : new Date(a.timestamp).getTime();
      const db = b.date_of_event ? new Date(b.date_of_event).getTime() : new Date(b.timestamp).getTime();
      return db - da; // desc (recent first)
    });
  }, [filtered, activeTab]);

  const effectiveList = activeTab === "upcoming" ? groupedFlattened : pastSorted;
  const total = effectiveList.length;
  const totalPages = Math.max(1, Math.ceil(total / Math.max(1, pageSize)));
  const currentPage = Math.min(page, totalPages);
  const startIndex = (currentPage - 1) * pageSize;
  const endIndex = Math.min(startIndex + pageSize, total);
  const pageItems = enablePagination ? effectiveList.slice(startIndex, endIndex) : effectiveList;
              ...
              <PopoverContent className="w-auto p-0 z-[80] max-w-[calc(100vw-2rem)]" align="start">
                <div className="p-3">
                  <div className="flex items-center gap-2 pb-3">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => setDateRange({})}>Clear</Button>
                  </div>
                  <Calendar
                    mode="range"
                  ...
                  <div className="flex justify-end gap-2 p-2 pt-3">
                    <Button variant="ghost" size="sm" onClick={() => setDateRange({ from: new Date() })}>From now</Button>
        ...
        {activeTab === "upcoming" ? (
                ...
                <section key={label}>
                  ...
                  {items.length > 0 ? (
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                      {items.map((submission) => (
                        <LinkComponent key={submission.id} href={getHref(submission)}>
                          <Card className="h-full transition hover:shadow-lg hover:ring-2 hover:ring-primary/40 dark:hover:shadow-[0_0_15px_rgba(0,0,0,0.6)]" suppressHydrationWarning>
                            <CardHeader>
                              ...
                              {submission.intended_actions && submission.intended_actions.length > 0 ? (
                                <div>
                                  ...
                                  {submission.intended_actions.length > 3 ? (
                                    <p className="mt-1 text-xs text-muted-foreground">+{submission.intended_actions.length - 3} more</p>
                                  ) : null}
                                </div>
                              ) : null}
                            </CardContent>
                            <CardFooter className="text-xs text-muted-foreground" suppressHydrationWarning>
                              <div className="flex flex-col gap-0.5">
                                {submission.date_of_event ? (
                                  <span>
                                    <span className="font-medium">Date of event:</span> {new Date(submission.date_of_event).toLocaleString()}
                                  </span>
                                ) : null}
                                <span>
                                  <span className="font-medium">Created:</span> {new Date(submission.timestamp).toLocaleString()}
                                </span>
                              </div>
                            </CardFooter>
                          </Card>
                    ...
                    <div className="rounded-md border p-4 text-sm text-muted-foreground">
                      {allInGroup.length === 0 ? "No Dispatches Available" : "No Dispatches in this section on this page"}
                    </div>
                  )}
                </section>
              );
          ...
          <section>
            ...
            {pageItems.length > 0 ? (
              ...
              <div className="rounded-md border p-4 text-sm text-muted-foreground">No past or archived events</div>
            )}
          </section>
        )}
      </div>
