...
import PodCard from "@workspace/ui/patterns/features/pods/pod-card";
...
export function PodsListLayout<TPod extends PodsListLayoutPod>({
  ...
  const [persistedFilters, setPersistedFilters] =
    useLocalStorage<PersistedPodsFilters>(storageKey, defaultPersistedFilters, {
      version: 2,
  ...
  React.useEffect(() => {
    ...
    if (typeof persistedFilters.query === "string")
      setQuery(persistedFilters.query);
    if (typeof persistedFilters.area === "string")
      setArea(persistedFilters.area);
    if (typeof persistedFilters.channel === "string")
      setChannel(persistedFilters.channel);
    if (
