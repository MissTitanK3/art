# 🗺️ Contributing Guide for `apps/region-stg`
...
1. **Keep region logic here**
   ...
   - Any DB or API adapters should live in `apps/region-stg/lib/`.

2. **Separation of concerns**
   - `@workspace/store` = business logic & state
   - `@workspace/ui` = reusable presentation components
   - `apps/region-stg` = glue code + region-specific DB/backend
...
- ❌ Don’t copy Zustand stores into `apps/region-stg`. They belong in `@workspace/store`.
