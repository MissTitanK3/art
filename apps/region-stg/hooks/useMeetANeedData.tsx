...
const MeetANeedQueryContext = createContext<QueryMeta>({ total: 0 });
export function useMeetANeedQueryMeta() {
  return useContext(MeetANeedQueryContext);
}
export function useMeetANeedData() {
  const setAll = useMeetANeedStoreProvider((s) => s.setAll);
  const [meta, setMeta] = useState<QueryMeta>({ total: 0 });
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        console.warn("[useMeetANeedData] failed to hydrate", e);
      }
    }
    run();
    return () => {
...
export function MeetANeedDataProvider({
  children,
}: {
  children: React.ReactNode;
}) {
  const { meta } = useMeetANeedData();
  return (
    <MeetANeedQueryContext.Provider value={meta}>
      {children}
    </MeetANeedQueryContext.Provider>
  );
}
