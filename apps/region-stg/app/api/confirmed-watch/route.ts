...
import { createSupabaseServerClient, createSupabaseWizardServiceClient } from '@/lib/auth/supabase/server';
...
export async function POST(req: Request) {
  try {
    ...
    const row = {
      ...
      agency_type: Array.isArray(payload?.agency_type) ? payload?.agency_type : null,
      ...
      officer_moving: typeof payload?.officer_moving === 'boolean' ? payload.officer_moving : null,
      officer_direction: (payload?.officer_direction ?? null) as Direction | null,
      lights_on: typeof payload?.lights_on === 'boolean' ? payload.lights_on : null,
      sirens_on: typeof payload?.sirens_on === 'boolean' ? payload.sirens_on : null,
      submitted_by,
      test: Boolean(payload?.test ?? false),
      updated_at: new Date().toISOString(),
    } as const;

    const wizardClient = createSupabaseWizardServiceClient();

    const { data: inserted, error } = await wizardClient.from('wizard').insert(row).select('*').single();
