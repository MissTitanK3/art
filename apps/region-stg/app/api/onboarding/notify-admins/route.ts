...
export async function POST() {
  try {
    ...
    const { data: rows, error: profErr } = await supabase
      .from("profiles")
      .select(
        "id, user_id, display_name, affiliation, city, state, access_role, verified_by, inserted_at, contact_signal, self_status_flags",
      )
    ...
    const needsOnboarding =
      !profile ||
      profile.verified_by === "self" ||
      profile.access_role === "team_member";
    if (!needsOnboarding) {
    ...
    if (!env.serviceRoleKey)
      return NextResponse.json(
        { error: "SERVER_MISSING_SERVICE_ROLE" },
        { status: 500 },
      );
    ...
    if (
      profile?.self_status_flags &&
      Array.isArray(profile.self_status_flags)
    ) {
    ...
    const windowHours = Number.parseInt(
      process.env.ONBOARDING_NOTIFY_WINDOW_HOURS || "24",
      10,
    );
    if (
      profile?.inserted_at &&
    ...
    const signal = profile?.contact_signal
    ...
    const { data: createdId, error: rpcErr } = await admin.rpc(
    ...
    try {
      const nextFlags = Array.isArray(profile?.self_status_flags)
        ? Array.from(
            new Set([
              ...(profile.self_status_flags as string[]),
      ...
      if (profile?.id) {
        await admin
          .from("profiles")
          .update({ self_status_flags: nextFlags as any })
          .eq("id", profile.id);
      }
    } catch {
      // non-blocking
