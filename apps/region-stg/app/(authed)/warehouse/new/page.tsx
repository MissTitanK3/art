...
import { WarehouseBuilderLayout } from "@workspace/ui/patterns/features/warehouse/warehouse-builder-layout";
import {
  WarehouseFormValues,
  WarehouseRecord,
  InventoryEntry,
...
function defaultZone(
  name: string,
  order: number
): WarehouseFormValues["zones"][number] {
  return {
    id: generateId(),
    name,
    sortOrder: order,
...
export default function WarehouseBuilderDataLayer() {
  ...
  const [visibility, setVisibility] = useState("private" as any);
  ...
  useEffect(() => {
    const fetchWarehouses = async () => {
      const client = getSupabaseBrowserClient();
      const { data: warehouses, error } = await client.from("warehouses")
        .select(`
                    *,
  ...
  const stewardLabel =
    activeStewardName.length > 0 ? activeStewardName : "Steward not set";
  ...
  const onSubmit = async (values: WarehouseFormValues) => {
    ...
    try {
      ...
      toast.success(
        isEditing ? "Warehouse updated" : "Warehouse synced to Dispatch"
      );
      router.push("/warehouse");
    } catch (error) {
      console.warn("[WarehouseBuilder] sync failed", error);
      toast.info(
        "Saved locally. Remote sync will retry once tables are ready."
      );
      router.push("/warehouse");
    } finally {
      setSyncing(false);
