 ...
 */
export async function createSupabaseServerClient(target: SupabaseTarget = 'region') {
  if (isDemoMode()) return createDemoSupabaseClient();
  const cookieStore = await cookies();
  const env = target === 'admin' ? ensureSupabaseEnv('admin') : ensureSupabaseEnv('server');
...
export function createSupabaseAdminServiceClient() {
  ...
  if (!env.serviceRoleKey) {
    throw new Error('Missing SUPABASE_SERVICE_ROLE_KEY_ADMIN for admin service operations.');
  }
...
export function createSupabaseRegionServiceClient() {
  if (isDemoMode()) return createDemoSupabaseClient();
