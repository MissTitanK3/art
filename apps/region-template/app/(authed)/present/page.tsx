...
import TeleprompterSettings from "@workspace/ui/patterns/features/teleprompter/teleprompter-settings";
import QuickActionsCard from "@workspace/ui/patterns/common/quick-actions-card";
import TeleprompterImportContent from "@workspace/ui/patterns/features/teleprompter/teleprompter-import-content";
    ...
    <div className="mx-auto grid max-w-6xl gap-4 px-2 md:px-0">
      ...
      <Card>
          ...
          <div className="flex flex-col gap-2 w-full">
            ...
            <div className="grid w-full grid-cols-2 md:grid-cols-4 gap-2">
              <Button
                className="w-full"
                variant="outline"
                onClick={() => setText(SCRIPTS[scriptId])}
              >
                Reset
              </Button>
              <Button
        ...
        <CardContent>
          {/* Playback controls */}
          <div
            className={`mb-2 flex flex-wrap items-center gap-2 md:gap-3 ${mobileControlsVisible ? "" : "hidden"} md:flex`}
          >
                      ...
                      onDismiss={() => {
                        ...
                        try {
                          localStorage.setItem(
                            "teleprompter.viewportHintDismissed",
                            "1",
                          );
                        } catch (err) {
                          console.warn(
                            "Failed to persist viewport hint dismissal",
                            err,
                          );
                        }
                      }}
        ...
        sections={[
          {
            ...
            actions: [
              ...
              {
                label: "Open Team Request (Scout)",
                variant: "outline",
                onClick: () => open("/team-req?eventType=scout_check"),
              },
            ],
          },
          {
            ...
            actions: [
              {
                ...
                variant: "outline",
                onClick: () => open("/dispatches"),
              },
              {
                label: "Open Schedules",
                variant: "outline",
                onClick: () => open("/schedules"),
              },
            ],
          },
          {
            ...
            actions: [
              {
                label: "Open Intake",
                onClick: () => open("/missing-persons/intake"),
              },
              {
                label: "Directory",
                variant: "outline",
                onClick: () => open("/missing-persons"),
              },
            ],
          },
          {
            ...
            actions: [
              {
                label: "Open Meet‑A‑Need",
                onClick: () => open("/meet-a-need"),
              },
            ],
          },
        ]}
      ...
      <Drawer open={importOpen} onOpenChange={setImportOpen}>
        <DrawerContent className="max-w-3xl bg-card text-card-foreground m-auto">
          ...
          <TeleprompterImportContent
            ...
            onSaveNow={() => bumpRev()}
            builtinScripts={TELEPROMPTER_SCRIPT_META.map((m) => ({
              id: m.id,
              label: m.label,
