...
export async function GET(req: Request) {
  try {
    ...
    const authorized =
      !!callerAccessRole && podAdmins.includes(callerAccessRole);
    if (!authorized)
    ...
    const scoped = profiles.filter(
      (p) => p?.access_role && allowed.has(p.access_role as any),
    );
    ...
    const result =
      field_role && field_role.trim().length > 0
        ? scoped.filter(
            (p: any) =>
              Array.isArray(p?.field_roles) &&
              p.field_roles.includes(field_role),
          )
        : scoped;
