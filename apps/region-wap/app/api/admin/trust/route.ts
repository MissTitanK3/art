...
import { getProfileByUserId, getTrustEntries, type TrustFilter } from '@/lib/dal/admin';
...
export async function GET(req: Request) {
  try {
    ...
    const signer_role = searchParams.get('signer_role');
    const status = searchParams.get('status');

    const filter: TrustFilter = {};
    if (signer_role) filter.signer_role = signer_role;
    if (status) filter.status = status;

    const { data, count } = await getTrustEntries(filter, page, pageSize);
