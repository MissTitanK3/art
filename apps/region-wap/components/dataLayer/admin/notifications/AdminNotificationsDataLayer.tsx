...
import AdminNotificationForm, { type SendArgs } from "@workspace/ui/components/admin/notifications/AdminNotificationForm";
...
export default function AdminNotificationsDataLayer() {
  const handleTemplate = React.useCallback(async (template: SendArgs['template']) => {
    try {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <Button variant="secondary" onClick={() => handleTemplate('maintenance')}>System Maintenance</Button>
          <Button variant="secondary" onClick={() => handleTemplate('dispatch_surge')}>Dispatch Surge</Button>
          <Button variant="secondary" onClick={() => handleTemplate('academy_reminder')}>Academy Reminder</Button>
          <Button variant="secondary" onClick={() => handleTemplate('welcome')}>Welcome Message</Button>
        </div>
        <hr className="my-2" />
