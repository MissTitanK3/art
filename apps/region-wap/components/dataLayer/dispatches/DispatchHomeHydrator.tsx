        ...
        if (!cancelled) {
          if (!submissionsRes.error && Array.isArray(submissionsRes.data)) {
            const deduped = Array.from(
              new Map(submissionsRes.data.map((r: any) => [String(r.id), mapRowToSubmission(r)])).values(),
            );
          ...
          // If both queries succeeded and returned empty arrays, make sure local state is cleared
          if (!submissionsRes.error && Array.isArray(submissionsRes.data) && submissionsRes.data.length === 0) {
            replaceSubmissions([]);
          }
          if (!shiftsRes.error && Array.isArray(shiftsRes.data) && shiftsRes.data.length === 0) {
            replaceShifts([]);
          }
        }
      } catch (e) {
        console.warn("[DispatchHomeHydrator] failed to hydrate home dashboard", e);
      }
    }
    hydrate();
    return () => {
