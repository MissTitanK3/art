# 🗺️ Contributing Guide for `apps/region-wap`
...
1. **Keep region logic here**  
   ...
   - Any DB or API adapters should live in `apps/region-wap/lib/`.  

2. **Separation of concerns**  
   - `@workspace/store` = business logic & state  
   - `@workspace/ui` = reusable presentation components  
   - `apps/region-wap` = glue code + region-specific DB/backend  
...
- ❌ Don’t copy Zustand stores into `apps/region-wap`. They belong in `@workspace/store`.  
