...
import { TELEPROMPTER_SCRIPTS as SCRIPTS, TELEPROMPTER_SCRIPT_META, TeleprompterScriptId as ScriptId } from "@workspace/ui/data/teleprompter-scripts";
...
export default function TeleprompterDataLayer() {
  ...
  const toggleFullscreen = React.useCallback(async () => { try { await toggleFs(); } catch { /* no-op */ } }, [toggleFs]);
  ...
  }, [mounted, importOpen, settingsOpen, focusViewport]);
  ...
  }, [mounted, storeScriptId, storeText, storeFontSize, storeLineHeight, storeMirrorH, storeMirrorV, storePreset, storeSpeed, storeDefaultSpeed, storeFontFace, storeCustomTextColor, storeCustomBgColor, storeCustomHighlightColor, storeOverlayColor, storeOverlayOpacity, storeCacheEnabled]);
                ...
                {mounted && !viewportHasFocus && !playing && !viewportHintDismissed && (
                  <KeyboardHint
                    ...
                    onDismiss={() => {
                      ...
                      try { localStorage.setItem('teleprompter.viewportHintDismissed', '1'); } catch { /* no-op */ }
                    }}
                  >
        ...
        <DrawerContent className="max-w-3xl bg-card text-card-foreground m-auto">
          ...
          <div className="grid gap-4 p-4">
            <TeleprompterSettings
              ...
              onResetKeyboardHint={() => { try { localStorage.removeItem('teleprompter.viewportHintDismissed'); } catch { /* no-op */ } setViewportHintDismissed(false); }}
