# 🗺️ Contributing Guide for `apps/region-pnw`
...
1. **Keep region logic here**  
   ...
   - Any DB or API adapters should live in `apps/region-pnw/lib/`.  

2. **Separation of concerns**  
   - `@workspace/store` = business logic & state  
   - `@workspace/ui` = reusable presentation components  
   - `apps/region-pnw` = glue code + region-specific DB/backend  
...
- ❌ Don’t copy Zustand stores into `apps/region-pnw`. They belong in `@workspace/store`.  
