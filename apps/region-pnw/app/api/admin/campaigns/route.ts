...
import { createSupabaseServerClient, createSupabaseAdminServiceClient } from '@/lib/auth/supabase/server'
...
export async function POST(req: Request) {
  try {
    ...
    const authorized = !!callerAccessRole && (regionAdmins.includes(callerAccessRole) || callerAccessRole === 'dispatcher_admin')
    ...
    const { data: adminProfRows } = await admin
      ...
      .limit(1)
    const callerRegionId = Array.isArray(adminProfRows) && adminProfRows[0]?.region_id ? String((adminProfRows as any)[0].region_id) : null

    const payload: any = {
      title,
      region_id: body.region_id ?? callerRegionId ?? null,
      start_at: body.start_at ?? new Date().toISOString(),
      end_at: body.end_at ?? new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString(),
      summary: body.summary ?? null,
      reward_schema: body.reward_schema ?? null,
      art_link: body.art_link ?? null,
    }

    const { data, error } = await admin.from('campaigns').insert(payload).select('*').limit(1)
...
export async function GET(_req: Request) {
  try {
    ...
    const { data: adminProfRows } = await admin
      ...
      .limit(1)
    const callerRegionId = Array.isArray(adminProfRows) && adminProfRows[0]?.region_id ? String((adminProfRows as any)[0].region_id) : null
    let q = admin.from('campaigns').select('*').gte('end_at', nowIso).order('start_at', { ascending: true })
    if (callerRegionId) q = q.or(`region_id.is.null,region_id.eq.${callerRegionId}`)
