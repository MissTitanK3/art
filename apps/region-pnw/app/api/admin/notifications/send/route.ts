...
function buildTemplate(t: NonNullable<Body['template']>): Pick<Body, 'title' | 'body' | 'level' | 'channel'> {
  switch (t) {
    case 'maintenance':
      return {
    ...
    default:
      return { title: 'Notification', body: '', level: 'info', channel: 'system' };
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (Array.isArray(body.recipientUserIds) && body.recipientUserIds.length > 0) {
      ...
      let prefs: Array<{ user_id: string; global_opt_out: boolean | null; muted_channels: string[] | null }> = [];
      try {
    ...
    return NextResponse.json({ ok: true, id: createdId ?? null, recipientsCount: recipients.length });
  } catch (e: any) {
    return jsonError(e);
  }
}
