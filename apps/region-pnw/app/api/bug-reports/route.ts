  ...
  try {
    ...
    (async () => {
      try {
        ...
        const allIds: string[] = (profileRows ?? []).map((r: any) => r.user_id).filter(Boolean);
        ...
        try {
          ...
          if (prefRows && prefRows.length > 0) {
            const muted = new Map<string, { global_opt_out: boolean; muted_channels: string[] }>();
        ...
        const { error: rpcErr } = await admin.rpc('create_notification_for_users', {
