...
import type { ComTeam, ComOperator, ComLog, ComChannel, ComBriefing, CommsMessageType, CommsImportance } from "@workspace/store/types/comms.ts";
...
export function useCommsData({ eventId }: UseCommsDataArgs) {
  ...
  React.useEffect(() => {
    ...
    async function initialFetch() {
      try {
        ...
        const [teamsRes, opsRes, logsRes, chansRes, briefRes] = await Promise.all([
  ...
  // Briefing upsert
  const upsertBriefing = React.useCallback(async (patch: Omit<ComBriefing, 'id' | 'event_id' | 'updated_at'>) => {
    const client = clientRef.current ?? getSupabaseBrowserClient();
    const nowIso = new Date().toISOString();
    if (briefing?.id) {
