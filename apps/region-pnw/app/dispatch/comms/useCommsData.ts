...
export function useCommsData({ eventId }: UseCommsDataArgs) {
  ...
  const [globalCheckInMinutes, setGlobalCheckInMinutes] = React.useState<number | undefined>(60);
  const clientRef = React.useRef<ReturnType<typeof getSupabaseBrowserClient> | null>(null);
    ...
    async function initialFetch() {
      try {
        ...
        const [teamsRes, opsRes, logsRes, chansRes, briefRes, alertsRes] = await Promise.all([
          ...
          client.from('com_briefings').select('*').eq('event_id', eventId).maybeSingle(),
        ...
        if (!cancelled) {
          ...
          setAlerts(Array.isArray(alertsRes.data) ? (alertsRes.data as any) : []);
        }
      } catch (e) {
    ...
    (log: {
      ...
      void (async () => {
        try {
          ...
          if (error) console.warn('[useCommsData] insert com_logs failed', error);
        } catch (e) {
  ...
  const checkInOperator = React.useCallback(async (id: string) => {
    ...
    setOperators((prev) => prev.map((op) => (op.id === id ? { ...op, last_check_in: nowIso } : op)));
    try {
      const client = clientRef.current ?? getSupabaseBrowserClient();
      const { error } = await client.from('com_operators').update({ last_check_in: nowIso }).eq('id', id);
      if (error) console.warn('[useCommsData] update com_operators last_check_in failed', error);
    } catch (e) {
      console.warn('[useCommsData] update com_operators last_check_in error', e);
    }
    return nowIso;
  }, []);
  ...
  const createTeam = React.useCallback(async (input: Omit<ComTeam, 'id' | 'created_at' | 'updated_at'>) => {
    ...
    const { error } = await client.from('com_teams').insert({
      ...
      default_check_in_interval_minutes: input.default_check_in_interval_minutes ?? null,
    });
  ...
  const updateTeam = React.useCallback(async (id: string, patch: Partial<ComTeam>) => {
    ...
    const { error } = await client.from('com_teams').update(payload).eq('id', id);
  ...
  const checkInTeam = React.useCallback(async (id: string) => {
    const nowIso = new Date().toISOString();
    setTeams((prev) => prev.map((t) => (t.id === id ? { ...t, last_check_in: nowIso } : t)));
    try {
      const client = clientRef.current ?? getSupabaseBrowserClient();
      const { error } = await client.from('com_teams').update({ last_check_in: nowIso }).eq('id', id);
      if (error) console.warn('[useCommsData] update com_teams last_check_in failed', error);
    } catch (e) {
    ...
    async (patch: Omit<ComBriefing, 'id' | 'event_id' | 'updated_at'>) => {
      ...
      if (briefing?.id) {
        ...
        const { error } = await client
          ...
          .eq('id', briefing.id);
        if (error) console.warn('[useCommsData] update com_briefings failed', error);
        return briefing.id;
      } else {
        const id = crypto.randomUUID();
        const record: ComBriefing = { id, event_id: eventId, updated_at: nowIso, ...patch } as ComBriefing;
        setBriefing(record);
        ...
        if (error) console.warn('[useCommsData] insert com_briefings failed', error);
        return id;
      }
    },
    [briefing, eventId],
  ...
  const createAlert = React.useCallback(
    async (input: { direction: string; description: string; id?: string }) => {
      ...
      try {
        ...
        if (error) console.warn('[useCommsData] insert com_alerts failed', error);
      } catch (e) {
  ...
  const updateAlert = React.useCallback(
    async (id: string, patch: Partial<Pick<ComAlert, 'direction' | 'description'>>) => {
      const nowIso = new Date().toISOString();
      setAlerts((prev) => prev.map((a) => (a.id === id ? { ...a, ...patch, updated_at: nowIso } : a)));
      try {
        const client = clientRef.current ?? getSupabaseBrowserClient();
        const payload: any = { ...patch, updated_at: nowIso };
        const { error } = await client.from('com_alerts').update(payload).eq('id', id);
        if (error) console.warn('[useCommsData] update com_alerts failed', error);
      } catch (e) {
