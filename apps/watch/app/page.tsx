...
import {
  AGENCY_GRADIENTS_DARK,
  AGENCY_GRADIENTS_LIGHT,
} from "@/constants/agencies";
const HeatOverlay = dynamic(
  () => import("@/components/map/HeatLayer").then((m) => m.HeatLayer),
  { ssr: false },
);
const SelectionOverlay = dynamic(
  () => import("@/components/map/SelectionOverlay"),
  { ssr: false },
);
...
function filterReportsBySettings(
  ...
  return reports.filter((report) => {
    ...
    const matchesTime =
      reportAgeHours >= minHours && reportAgeHours <= maxHours;
    const matchesAgency =
      isAllAgencies ||
      (Array.isArray(report.agency_type) &&
        report.agency_type.some((a) => agencyFilter.includes(a)));
...
export default function Home() {
      ...
      if (storedRange) {
        ...
        if (Array.isArray(parsed) && parsed.length === 2) {
          ...
          const endCandidate = Math.min(
            Math.max(0, Number(parsed[1]) || 168),
            168,
          );
  ...
  useEffect(() => {
    ...
    const update = () => {
      setGeoPermissionState(
        (status?.state as "granted" | "prompt" | "denied" | null) ?? null,
      );
    };
    (async () => {
      try {
  ...
  return (
    <>
      ...
      <div className={mapContainerClass}>
        ...
        <div
          className={`absolute inset-0 ${selecting ? "ring-2 ring-amber-300/40" : ""}`}
        >
          <MapWrapper
            position={position}
            zoom={zoom}
      ...
      <WizardDrawer
        ...
        onSubmit={async () => {
          ...
          if (draft.lights_on != null)
            fd.append("lights_on", String(!!draft.lights_on));
          if (draft.sirens_on != null)
            fd.append("sirens_on", String(!!draft.sirens_on));
          if (draft.officer_moving != null)
            fd.append("officer_moving", String(!!draft.officer_moving));
          if (draft.officer_direction)
            fd.append("officer_direction", draft.officer_direction);
        ...
        description={
          <div className="space-y-2">
            ...
            <p className="text-white/70">
              This is temporary and will turn off after you submit.
            </p>
          </div>
        }
      ...
      <ChoiceModal
        ...
        description={
          <>
            <p>
              You can either use your current location (auto-detect) or tap
              anywhere on the map to select a spot manually for your report.
            </p>
            <p>
              To submit a test report (for practice or demo), enable test mode
              in the report options after choosing your location. Test reports
              are not included in public data.
            </p>
          </>
        }
        ...
        onSecondary={() => {
          ...
          if (userPos) setShowRadius(true);
          else setShowRadius(false);
          setSelecting(true);
          handleFindMe({ force: true });
          setLocMode("report");
          setZoom(15);
      ...
      <ConfirmModal
        ...
        description={
          <div className="space-y-2">
            <p>
              Your browser is blocking location access. To enable, click the
              site permissions icon (lock) near the address bar and allow
              Location, then try again.
            </p>
            <p className="text-white/70 text-sm">
              On mobile, check your browser or OS settings for this site.
            </p>
          </div>
        }
