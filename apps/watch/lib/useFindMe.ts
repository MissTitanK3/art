...
export function useFindMe(
  ...
  const handleFindMe = useCallback(
    (opts?: { force?: boolean }) => {
      ...
      const now = Date.now();
      const shouldFetch = opts?.force
        ? true
        : !cachedCoords || now - timestamp > refreshIntervalMs;
      ...
      const success = (pos: GeolocationPosition) => {
        ...
        const movedEnough =
          !cachedCoords ||
          getDistanceM(cachedCoords, coords) > movementThresholdM;
      ...
      const failure = () => {
        const fallbackMessage =
          getTranslation?.("locationDenied") ?? "Unable to retrieve location.";
