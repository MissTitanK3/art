...
export default function BottomNav() {
  ...
  const { step, back, next, canContinue, finish, testReportEnabled } =
    useWizard();
  ...
  const wizardButtonBase =
    "flex items-center gap-1 px-4 py-2 rounded-full shadow backdrop-blur-md transition border";

  const wizardButtonActive =
    "bg-blue-500/50 text-white border-blue-300 hover:bg-blue-500/60";

  const wizardButtonDisabled =
    "bg-white/20 text-white/40 border-white/30 cursor-not-allowed";
  ...
  return (
    <nav className={baseNavClass}>
      <Link
        href="/"
        className="flex flex-col items-center text-sm text-white hover:text-blue-200 transition"
      >
