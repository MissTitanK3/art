  ...
  const renderLine = (line: string, idx: number) => {
    ...
    return (
      <div key={idx} className="leading-relaxed font-mono">
        ...
        {keyMatch && (
          <span className="text-yellow-300">{`"${keyMatch[1]}"`}</span>
        )}
...
export default function LanguageSupportPanel({
  compact,
}: LanguageSupportPanelProps) {
  ...
  return (
    <div className={`text-white ${textSize} ${sectionGap}`}>
      {!compact && (
        <h2 className="text-2xl font-bold">🌍 {t("requestLanguageSupport")}</h2>
      )}

      <p className="text-white/80">
        {t("languageSupportInvite")}{" "}
        <a
          href="mailto:icetea@peoplesrebellion.org"
      ...
      <section className="space-y-2">
        ...
        {template ? (
          ...
          <p className="text-sm text-white/60">Loading template...</p>
        )}
      </section>
