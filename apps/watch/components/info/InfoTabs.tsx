...
export default function InfoTabs() {
  ...
  const tabs = useMemo(
    () => [
      { id: 'join' as TabKey, label: t('joinDispatchTitle'), content: <JoinDispatchPanel /> },
      { id: 'language' as TabKey, label: t('requestLanguageSupport'), content: <LanguageSupportPanel compact /> },
      { id: 'resources' as TabKey, label: t('immigrantResourcesTitle'), content: <GroupedImmigrantResources /> },
      { id: 'transparency' as TabKey, label: t('transparencyTitle'), content: <TransparencyPanel /> },
    ],
    [t],
  );
      ...
      <div className="flex flex-wrap gap-2 text-xs sm:text-sm">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-3 py-1 rounded-full border transition ${activeTab === tab.id
