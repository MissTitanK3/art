...
function VisibleReportsTracker({
  reports,
  onChange,
}: {
  reports: Report[];
  onChange: (visible: Report[]) => void;
}) {
...
export function HeatLayer({
  ...
  reports: Report[];
  gradients: Record<string, Record<number, string>>;
}) {
  const map = useMap();
    ...
    for (const [agency, points] of Object.entries(grouped)) {
      const heat = (L as any)
        .heatLayer(points, {
          ...
          blur: 20,
          maxZoom: 17,
          maxOpacity: 0.9, // keep centers opaque
          minOpacity: 0.4, // ensure they’re visible even at low intensity
          gradient: gradients[agency],
        })
        .addTo(map);
...
export default function HeatMap({
  ...
  const gradients =
    tile.mode === "dark" ? AGENCY_GRADIENTS_DARK : AGENCY_GRADIENTS_LIGHT;
    ...
    <div className="rounded overflow-hidden z-0">
      <div className="relative" ref={mapContainerRef}>
        <div className="h-[500px]">
          ...
          >
            ...
            <HeatLayer reports={visibleReports} gradients={gradients} />
            <VisibleReportsTracker
              reports={reports}
              onChange={setVisibleReports}
            />
      ...
      <div className="flex my-2 gap-3 flex-wrap justify-center text-center">
        ...
        <div className="flex my-2 gap-3 flex-wrap justify-center text-center">
          ...
          <FrostedButton
            className="w-56"
            onClick={() => mapRef.current?.setZoom(8)}
          >
            {t("mapControlsRegion")}
          </FrostedButton>
          <FrostedButton
            className="w-56"
            onClick={() => mapRef.current?.setZoom(3)}
          >
            {t("mapControlsUS") ?? "US"}
          </FrostedButton>
        </div>
        ...
        {Object.entries(AGENCY_GRADIENTS).map(([agency, gradient]) => (
          <li key={agency} className="flex items-center gap-2">
            <span
              ...
              style={{ backgroundColor: gradient[0.4] }}
            />
            <span>{agency}</span>
          </li>
        ))}
        ...
        {visibleReports.length === 0 ? (
          ...
          <ul className="space-y-4">
            {visibleReports.map((r) => {
              const primaryAgency =
                r.agency_type?.[0] || r.agency_other || "Other";
              const translatedAgencies = (r.agency_type || []).map((a) =>
                t(`agency.${a}` as TranslationKey),
              );
                ...
                >
                  ...
                  >
                    <div className="mb-1 text-2xl">
                      {[...translatedAgencies, r.agency_other]
                        .filter(Boolean)
                        .join(", ")}
                    </div>
                    {r.test && (
                  ...
                  <div
                    className={`p-4 space-y-2 text-sm ${r.test ? "bg-red-900" : ""} text-white/90`}
                  >
                    <div>
                      ...
                      {r.officer_moving === true
                        ? r.officer_direction
                          ? t(
                              `direction.${r.officer_direction}` as TranslationKey,
                            )
                    ...
                    <div className="flex w-full justify-evenly flex-wrap gap-4">
                      <div>
                        <strong>{t("lightsOn")}:</strong>{" "}
                        {r.lights_on ? t("yes") : t("no")}
                      </div>
                      <div>
                        <strong>{t("sirensOn")}:</strong>{" "}
                        {r.sirens_on ? t("yes") : t("no")}
                      </div>
                    </div>
