...
export default function FeedDrawer({ zoom, openAtZoom = 12, openExternal, onCloseExternal, reports, onZoomTo }: FeedDrawerProps) {
  ...
  useEffect(() => {
    ...
    if (!open && !Number.isNaN(last) && last < openAtZoom && zoom >= openAtZoom) {
      setOpen(true);
    }
...
function FeedContent({ reports, onZoomTo }: { reports?: Report[]; onZoomTo?: (pos: { lat: number; lng: number }) => void }) {
  const { t } = useTranslations();
