  ...
  return (
    <div className="space-y-6">
      <section className="space-y-3">
        ...
        <p className="text-xs text-white/70 mt-2">{t('joinDispatchRegionNote')}</p>
      </section>

      <section>
        <h4 className="text-lg font-semibold mb-3">🚨 {t('joinDispatchRolesTitle')}</h4>
        <ul className="grid grid-cols-1 gap-2">
          {roleKeys.map((key) => (
            <li key={key} className="bg-white/10 rounded-md px-3 py-2 border border-white/15 text-white/95">
              {t(key)}
            </li>
          ))}
        </ul>
