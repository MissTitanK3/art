...
import { Plus, Map as MapIcon, Settings, RefreshCw, Languages, Compass, BookOpen, ArrowLeftFromLine, ArrowRightFromLine } from 'lucide-react';
...
export default function FABStack({ onAddReport, onFilters, onToggleLive, onInfo, liveActive, unit, onToggleUnit, onMapSettings, isReporting, onCancelReport }: Props) {
  ...
  if (typeof window !== 'undefined' && !compact && localStorage.getItem('fab_compact') === '1') {
  ...
  const setCompactPersist = (v: boolean) => {
    setCompact(v);
    try { localStorage.setItem('fab_compact', v ? '1' : '0'); } catch { }
  };

  const label = (icon: React.ReactNode, text: string) =>
    compact ? (
      <span className="inline-flex items-center gap-2" aria-label={text} title={text}>
        {icon}
      </span>
    ) : (
    ...
    <div className="fixed bottom-6 right-4 z-[43] flex flex-col items-end gap-2">
      {open && (
        <div className="flex flex-col items-end gap-2 mb-18">
          ...
          <button
            onClick={() => { onToggleLive?.(); setOpen(false); }}
            ...
            {label(<RefreshCw className="w-4 h-4" />, liveActive ? t('liveOn') : t('liveOff'))}
          </button>
          <button
            onClick={() => { setLanguage(language === 'en' ? 'es' : 'en'); setOpen(false); }}
          ...
          <button
            onClick={() => { onToggleUnit?.(); setOpen(false); }}
            className={`px-4 py-2 rounded-full bg-zinc-700 text-white shadow hover:bg-zinc-600 ${compact ? 'w-20' : 'w-44'}`}>
            {label(
          ...
          <button onClick={() => { onInfo?.(); setOpen(false); }} className={`px-4 py-2 rounded-full bg-zinc-700 text-white shadow hover:bg-zinc-600 ${compact ? 'w-20' : 'w-44'}`}>
            {label(<BookOpen className="w-4 h-4" />, t('info'))}
          </button>
          <button
            onClick={() => setCompactPersist(!compact)}
            className={`px-4 py-2 rounded-full bg-zinc-700 text-white shadow hover:bg-zinc-600 ${compact ? 'w-20' : 'w-44'}`}>
            {compact ? label(<ArrowLeftFromLine className="w-4 h-4" />, t('showLabels')) : label(<ArrowRightFromLine className="w-4 h-4" />, t('iconsOnly'))}
          </button>
