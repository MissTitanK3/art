# 🗺️ Contributing Guide for `apps/region-responder`
...
1. **Keep region logic here**
   ...
   - Any DB or API adapters should live in `apps/region-responder/lib/`.

2. **Separation of concerns**
   - `@workspace/store` = business logic & state
   - `@workspace/ui` = reusable presentation components
   - `apps/region-responder` = glue code + region-specific DB/backend
...
- ❌ Don’t copy Zustand stores into `apps/region-responder`. They belong in `@workspace/store`.
