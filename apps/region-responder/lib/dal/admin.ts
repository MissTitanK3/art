...
export async function getProfiles(
  ...
  try {
    ...
    if (serviceKey) {
      try {
        ...
        if (authorized) {
          ...
          let adminQuery = adminClient.from('profiles').select('*', { count: 'exact' });
          ...
          if (filter?.query) {
            ...
            adminQuery = adminQuery.or(`display_name.ilike.${q},email.ilike.${q},handle.ilike.${q}`);
          }

          const from = (page - 1) * pageSize;
          const to = from + pageSize - 1;
          adminQuery = adminQuery.range(from, to);

          const { data, error, count } = await adminQuery;
          if (error) throw error;
          const rows = Array.isArray(data) ? data : [];
          return { data: rows as unknown as Profile[], count: count ?? 0 };
        }
      } catch (e) {
    ...
    if (filter?.query) {
      const q = `%${filter.query}%`;
      query = query.or(`display_name.ilike.${q},email.ilike.${q},handle.ilike.${q}`);
    }
...
export async function getPods(page: number = 1, pageSize: number = 50): Promise<{ data: Pod[]; count: number }> {
  ...
  try {
    ...
    try {
      ...
      const { data, error, count } = await client
        .from('pods')
        .select('id, slug, name, area, channels', { count: 'exact' })
        .is('deleted_at', null)
        .range(from, to);
      if (error) throw error;
      const rows = Array.isArray(data) ? data : [];
...
export async function getTrustEntries(
  filter?: TrustFilter,
  page: number = 1,
  pageSize: number = 50,
): Promise<{ data: TrustEntry[]; count: number }> {
  try {
    const client = await createSupabaseServerClient();
    let query = client.from('trust_signatures').select('*', { count: 'exact' }).is('deleted_at', null);

    if (filter?.signer_role) query = query.eq('signer_role', filter.signer_role);
...
export async function getUserPermissionsContext(profileId: string) {
  ...
  // Fetch user's orgs
  const { data: orgData } = await client.from('organization_roles').select('org_id').eq('user_id', profileId);
