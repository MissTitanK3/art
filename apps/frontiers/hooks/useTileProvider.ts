...
import { DEFAULT_TILE_PROVIDER, TILE_PROVIDER_STORAGE_KEY, TILE_PROVIDERS, type TileProvider } from '@/lib/map/tiles';
...
export function useTileProvider() {
  const [providerId, setProviderIdState] = useState<string>(() => {
    ...
    return TILE_PROVIDERS.some((p) => p.id === stored) ? (stored as string) : DEFAULT_TILE_PROVIDER.id;
  });
