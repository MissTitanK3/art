...
export function ResonanceMeter() {
      ...
      try {
        ...
        if (profileId) {
          const { data: effects } = await supabase
            ...
            .select('id, expires_at, created_at, source_email, source_id, hop, strength')
        ...
        if (email || profileId) {
          let query = supabase
            .from('resonance_effects')
            .select('id, created_at, hop, strength, source_email, source_id, recipient_id, amount, expires_at')
            .order('created_at', { ascending: false })
            .limit(10)
          if (email && profileId) {
            query = query.or(`source_email.eq.${email},recipient_id.eq.${profileId}`)
          } else if (email) {
          ...
          if (active) {
            const items: FeedItem[] = (data || []).map((r: any) => {
              ...
              const boost = r.strength ? `→ +${Math.round((r.strength || 0) * 5)}% support` : ''
              return {
                id: r.id,
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'donations', filter: `profile_email=eq.${email}` }, (payload) => {
        const amount = (payload.new as any)?.amount
        toast.success(`Ko-fi donation detected. Pulse triggered (+${amount}).`)
      })
      .subscribe()
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'resonance_effects', filter: `source_email=eq.${email}` }, () => {
        setAffected((n) => n + 1)
      })
      .subscribe()
    return () => { try { supabase.removeChannel(ch) } catch { } }
  ...
  const kofiUrl = useMemo(() => process.env.NEXT_PUBLIC_KOFI_URL || 'https://ko-fi.com', [])

  return (
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        <span>Active Pulse</span>
        <span className="font-mono tabular-nums">{fmtCountdown(nextExpireAt)}{nowTick ? '' : ''}</span>
      </div>
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogTrigger asChild>
          <Button size="sm" variant="default">Trigger Pulse</Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-md bg-card/95 backdrop-blur">
          ...
          <div className="space-y-3 text-sm">
            ...
            <a className="inline-flex items-center justify-center rounded-md bg-primary px-3 py-2 text-primary-foreground hover:opacity-90" href={kofiUrl} target="_blank" rel="noreferrer">
              Donate via Ko‑fi
            </a>
            <div className="mt-4 space-y-2">
              <div className="text-xs uppercase tracking-wide text-muted-foreground">Recent Resonance</div>
              <ul className="space-y-1">
                {feed.map((f) => (
                  <li key={f.id} className="text-xs flex items-center gap-2">
                    <span className="text-muted-foreground w-14">[{f.when}]</span>
                    <span>{f.text}</span>
                  </li>
                ))}
                {feed.length === 0 ? (
                  <li className="text-xs text-muted-foreground">No recent pulses</li>
                ) : null}
              </ul>
            </div>
          </div>
