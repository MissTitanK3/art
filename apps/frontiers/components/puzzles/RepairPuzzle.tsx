...
export function RepairPuzzle({ signal }: { signal: ArtSignal }) {
  ...
  async function complete() {
    ...
    try {
      ...
      await supabase.from('art_signals').update({ is_discovered: true }).eq('id', signal.id)
      // Phase 2: Faction reputation increment
      const factionId = useProfileStore.getState().region_id || process.env.NEXT_PUBLIC_REGION_ID || 'demo-region'
      const multiplier = getDifficultyMultiplier(signal)
      useFactionStore.getState().incrementReputation(String(factionId), 5 * multiplier)
      // Engineering XP gain
      ...
      try { useAchievementsStore.getState().onRepair() } catch {}
      setDone(true)
    } catch (e: any) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <Switch checked={calibrate} onCheckedChange={(v) => onToggle('Calibrate', !!v)} />
      </div>
      <div className="flex items-center justify-between">
        <span className="text-sm">Stabilize</span>
        <Switch checked={stabilize} onCheckedChange={(v) => onToggle('Stabilize', !!v)} />
      </div>
      <div className="flex items-center justify-between">
        <span className="text-sm">Confirm</span>
        <Switch checked={confirm} onCheckedChange={(v) => onToggle('Confirm', !!v)} />
      </div>

      {done ? (
        <div className="text-xs text-green-600 dark:text-green-400">Repair complete.</div>
      ) : (
        <div className="text-xs text-muted-foreground">Toggle in order: Calibrate → Stabilize → Confirm.</div>
      )}
...
function labelForSignal(signal: ArtSignal) {
  ...
  if (s.includes('class') || s.includes('assembly')) return 'Assembly completed'
  if (s.includes('session') || s.includes('cache')) return 'Knowledge Cache decoded'
