...
async function syncNow() {
  ...
  try {
    const res = await fetch('/api/campaigns/progress', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ rows }) })
...
export function MissionsSyncAgent() {
  useEffect(() => {
    const id = setInterval(() => { void syncNow() }, 10 * 60 * 1000)
    const before = () => { void syncNow() }
