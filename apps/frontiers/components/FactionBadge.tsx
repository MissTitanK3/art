...
export function FactionBadge() {
  const regionId = useProfileStore((s) => s.region_id) || process.env.NEXT_PUBLIC_REGION_ID || 'demo-region'
  const rep = useFactionStore((s) => s.getReputation(regionId))
  const rank = useMemo(() => rankFor(rep), [rep])
  const variant = rank === 'Ally' ? 'success' : rank === 'Agent' ? 'info' : rank === 'Associate' ? 'secondary' : 'outline'
  return (
