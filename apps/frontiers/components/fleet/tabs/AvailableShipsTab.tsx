...
export function AvailableShipsTab(props: {
  ...
  return (
    <section id="available-ships">
      <h2 className="text-sm font-medium text-muted-foreground mb-2">Available Ships</h2>
      {catalogLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {catalog.map((s) => {
            ...
            return (
              <div key={s.id} className={`rounded border p-3 space-y-2 ${selected ? 'ring-2 ring-primary' : ''}`}>
                <div className="flex items-center justify-between">
                  <div className="font-medium text-sm">{s.name}</div>
                  <div className="text-xs text-muted-foreground">Tier {s.tier}</div>
                </div>
                <div className="text-xs text-muted-foreground">{s.description}</div>
                <div className="text-xs">Unlocks after {s.required_days} day(s)</div>
                <div className="flex justify-end">
                  <Button size="sm" variant={selected ? 'secondary' : 'default'} disabled={!s.eligible || selected} onClick={() => selectShip(s.id)}>
