...
type Kind = { id: string; name: string; description?: string; tier?: number; base?: Record<string, number>; perLevel?: Record<string, number>; upgradeCostBase?: number; upgradeCostGrowth?: number; replaceCost?: number }
...
export function ComponentsTab(props: {
  ...
  setConfirmUpgrade: (v: { slot: ShipComponent['slot']; cost: number } | null) => void
  confirmReplace: { slot: ShipComponent['slot']; kindId: string; cost: number; deltas: Array<[string, number, number]> } | null
  setConfirmReplace: (v: { slot: ShipComponent['slot']; kindId: string; cost: number; deltas: Array<[string, number, number]> } | null) => void
  ...
  return (
    <section>
      <h2 className="text-sm font-medium text-muted-foreground mb-2">Ship Components</h2>
      {!profileId ? (
        <div className="text-sm text-muted-foreground">Sign in to view components.</div>
      ) : componentsLoading ? (
        ...
        <div className="rounded border p-3 text-sm flex items-center justify-between gap-3">
          <span className="text-muted-foreground">No components installed yet.</span>
          <Button size="sm" onClick={installComponents} disabled={!currentShip}>Install Components</Button>
        </div>
      ) : (
        <>
          {missingSlots.length > 0 ? (
            <div className="rounded border p-2 text-[11px] text-muted-foreground">
              Missing {missingSlots.length} component{missingSlots.length > 1 ? 's' : ''}. Install per slot below or use Install Components to add all.
            </div>
          ) : null}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {missingSlots.map((slot) => {
              ...
              return (
                <MissingSlotCard key={`missing-${slot}`} slot={slot} kind={k as any} onInstall={() => installComponentForSlot(slot)} disabled={!defaultKind} />
              )
            })}
            {components.map((c) => {
              const kinds = catalogKinds[c.slot] || []
              return (
                <ComponentCard key={c.id} c={c} kinds={kinds as any} onUpgrade={() => prepareUpgradeConfirm(c)} onReplace={() => setReplaceOpen({ slot: c.slot })} />
              )
            })}
          </div>
          <Dialog open={!!replaceOpen} onOpenChange={(o) => setReplaceOpen(o ? replaceOpen : null)}>
            <DialogContent className="sm:max-w-md bg-card text-card-foreground">
              ...
              {replaceOpen?.slot ? (
                <div className="space-y-2 text-sm">
                  <div className="text-xs text-muted-foreground">Select a new kind for slot: <span className="font-medium">{replaceOpen.slot}</span></div>
                  <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-auto">
                    {(catalogKinds[replaceOpen.slot] || []).map((k) => (
                      <button
                        ...
                        onClick={() => {
                          const cur = components.find((x) => x.slot === replaceOpen.slot)
                          if (!cur) return
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[10px] text-muted-foreground">{k.id}</span>
                        </div>
                        {k.description ? (
          ...
          <ConfirmUpgradeDialog
            ...
            onConfirm={async () => { if (confirmUpgrade) { await doUpgrade(confirmUpgrade.slot); setConfirmUpgrade(null) } }}
          />
          <ConfirmReplaceDialog
            ...
            onConfirm={async () => { if (confirmReplace) { await doReplace(confirmReplace.slot, confirmReplace.kindId) } }}
          />
        </>
      )}
    </section>
