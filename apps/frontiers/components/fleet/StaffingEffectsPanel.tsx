    ...
    <div className="border rounded p-2 text-xs text-muted-foreground space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            aria-label="Open staffing breakdown"
        ...
        {derivedBreakdown?.auras?.length ? (
          <div className="flex flex-wrap gap-1">
            {derivedBreakdown.auras.map((a, i) => (
              <span
                key={i}
                className="rounded bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 px-1.5 py-0.5"
              >
                {a}
              </span>
            ))}
      ...
      {derivedBreakdown?.items?.length ? (
        <details className="mt-1">
          <summary className="cursor-pointer text-[11px] text-foreground/80">
            Breakdown
          </summary>
            ...
            {derivedBreakdown.items.map((it, idx) => (
              ...
              >
                <span className="text-[11px]">
                  {it.type === "crew" ? "Crew" : "Position"}: {it.name || it.id}
                </span>
                <div className="flex flex-wrap gap-1">
