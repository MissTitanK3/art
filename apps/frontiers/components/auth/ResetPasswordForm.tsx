      ...
      <CardContent>
        <form className="space-y-3" onSubmit={onSubmit}>
          <div className="space-y-1">
            ...
            <PasswordInput value={password} onChange={setPassword} name="new-password" autoComplete="new-password" />
          </div>
          <div className="space-y-1">
            <Label>Confirm password</Label>
            <PasswordInput value={confirm} onChange={setConfirm} name="confirm-password" autoComplete="new-password" />
          </div>
          {error ? <p className="text-xs text-destructive/80">{error}</p> : null}
          <Button className="w-full">Update password</Button>
          {ok ? <p className="text-xs text-green-600 dark:text-green-400">Password updated. You can now continue.</p> : null}
        </form>
      </CardContent>
      <CardFooter />
    </Card>
