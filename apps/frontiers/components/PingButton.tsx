...
export function PingButton() {
  const { setSignals, setLocation, setLoading, setError, signals, loading, location, error } = useSignalsStore()
  ...
  const onPing = useCallback(async () => {
    ...
    try {
      ...
      const json = profileId ? await fetchCurrentShipCached(profileId, 60_000) : null
  ...
  }, [setError, setLoading, setLocation, setSignals, regionIdFromStore, sectorFromStore])
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full max-w-2xl">
      ...
      {location && (
        <p className="text-xs text-muted-foreground">Location ~ {location.lat.toFixed(2)}, {location.lng.toFixed(2)}</p>
      )}
      {/* Results */}
      {signals?.length > 0 && (
        <>
      ...
      {!loading && signals.length === 0 && (
        <p className="text-xs text-muted-foreground">No signals yet. Try pinging.</p>
      )}
      {!loading && error && <p className="text-xs text-destructive/80">{error}</p>}
    </div>
  )
}
