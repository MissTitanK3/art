...
function Meter({ label, value }: { label: string; value: number }) {
  const color = useMemo(() => (value <= 33 ? 'bg-green-500' : value <= 66 ? 'bg-yellow-500' : 'bg-red-500'), [value])
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] text-muted-foreground w-20 text-left">{label}</span>
