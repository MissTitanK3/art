...
const DEFINITIONS: AchievementDef[] = [
  {
    id: "first_repair",
    title: "First Repair",
    trigger: "first_repair",
    reward: { xp: 25, badge: "wrench" },
  },
  {
    id: "first_pulse",
    title: "First Resonance Pulse",
    trigger: "first_pulse",
    reward: { xp: 25, badge: "wave" },
  },
  {
    id: "rest_streak_7",
    title: "Rested 7 Days Straight",
    trigger: "rest_streak_7",
    reward: { xp: 50, badge: "moon" },
  },
  {
    id: "season_master",
    title: "Season Master",
    trigger: "season_master",
    reward: { xp: 100, badge: "laurel" },
  },
];
  ...
  persist(
    (set, get) => ({
      ...
      award: (id) => {
        ...
        set((state) => ({
          unlocked: { ...state.unlocked, [id]: { id, earnedAt: now } },
        }));
      },
      onRepair: () =>
        set((state) => {
          ...
          if (repairs === 1)
            setTimeout(
              () => useAchievementsStore.getState().award("first_repair"),
              0,
            );
      ...
      onPulse: () =>
        set((state) => {
          ...
          if (pulses === 1)
            setTimeout(
              () => useAchievementsStore.getState().award("first_pulse"),
              0,
            );
      ...
      onRest: () =>
        set((state) => {
          ...
          const last = state.stats.rest.lastDate
            ? new Date(state.stats.rest.lastDate)
            : null;
          ...
          if (
            streak >= 7 &&
            !useAchievementsStore.getState().isUnlocked("rest_streak_7")
          ) {
            setTimeout(
              () => useAchievementsStore.getState().award("rest_streak_7"),
              0,
            );
          }
          return {
            stats: {
              ...state.stats,
              rest: { lastDate: today.toISOString(), streak },
            },
          };
        }),
      checkSeasonComplete: (campaignId: string) => {
        try {
          ...
          set((state) => {
            ...
            // award once
            setTimeout(
              () => useAchievementsStore.getState().award("season_master"),
              0,
            );
            return {
              stats: {
