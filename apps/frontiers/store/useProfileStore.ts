...
type ProfileState = {
  ...
  setFatigue: (kind: 'engineering' | 'navigation' | 'operations', value: number) => void;
...
export const useProfileStore = create<ProfileState>()(
  persist(
    (set) => ({
      ...defaults,
      setProfile: (p) => set({ profile: p, region_id: p?.region_id ?? null, sector_code: p?.sector_code ?? null }),
      setRegion: (id) => set({ region_id: id }),
      setSector: (sector) => set({ sector_code: sector }),
      addEngineeringXp: (amount) =>
        set((state) => ({ engineering_xp: Math.max(0, Math.round((state.engineering_xp ?? 0) + amount)) })),
      ...
      setDock: (lat, lng, radiusKm) =>
        set(() => ({
          ...
          dock_radius_km: typeof radiusKm === 'number' ? Math.max(0, Math.round(radiusKm * 1e4) / 1e4) : 0.4023,
        })),
      clearDock: () => set({ dock_lat: null, dock_lng: null, dock_radius_km: null }),
