...
export function isKindAllowed(slot: ShipComponent['slot'], kind: string): boolean {
  return getKindsForSlot(slot).some((k) => k.id === kind);
}

export function getKind(slot: ShipComponent['slot'], kind: string): ComponentKind | undefined {
  return getKindsForSlot(slot).find((k) => k.id === kind);
}

export function computeKindStats(kind: ComponentKind | undefined, level: number): Record<string, number> {
  if (!kind) return {};
  const base = kind.base || {};
  const per = kind.perLevel || {};
  const lvl = Math.max(1, Number(level || 1));
  const extra = lvl > 1 ? Object.fromEntries(Object.entries(per).map(([k, v]) => [k, (v || 0) * (lvl - 1)])) : {};
  const out: Record<string, number> = { ...base };
  for (const [k, v] of Object.entries(extra)) out[k] = (out[k] || 0) + (v || 0);
  return out;
}

export function computeUpgradeCost(kind: ComponentKind | undefined, currentLevel: number): number {
  if (!kind) return 0;
