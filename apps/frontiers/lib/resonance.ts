...
export async function triggerResonance(args: DonationArgs) {
  ...
  while (hop < MAX_HOP && inserted < CAP && currentIds.length > 0) {
    ...
    // Apply trust weights when UUIDs are used in connections_v2
    const uuidCurrent = currentIds.filter((id) => /[0-9a-f-]{36}/i.test(id));
    const uuidSlice = slice.filter((id) => /[0-9a-f-]{36}/i.test(id));
