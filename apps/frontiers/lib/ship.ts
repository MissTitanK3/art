...
export async function fetchShipState(profileId: string) {
  ...
  const { data, error } = await supabase.from('ship_states').select('*').eq('profile_id', profileId).maybeSingle();
...
export async function resetShipState(profileId: string) {
  ...
  const { data, error } = await supabase.rpc('reset_state', { profile_id: profileId });
  if (error) throw error;
  return data as ShipState | null;
}
