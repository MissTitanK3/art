...
export default function SeasonsPage() {
  const regionId = useProfileStore(
    (s) =>
      s.region_id ||
      s.profile?.region_id ||
    ...
    <section className="space-y-4">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Join a seasonal campaign to modify missions, colors, and rewards.
        </p>
      </header>
          ...
          return (
            <li key={c.id}>
              <Card>
                ...
                <CardContent>
                  {c.summary ? (
                    <p className="text-sm mb-2">{c.summary}</p>
                  ) : null}
                  ...
                  {(() => {
                    ...
                    return (
                      <ul className="space-y-2 mb-3">
                        {missions.map((m) => {
                          const req = (m.required_actions || {}) as Record<
                            string,
                            number
                          >;
                  ...
                  <div className="flex gap-2">
                    {!joined ? (
                      <Button size="sm" onClick={() => setActiveCampaign(c)}>
                        Join Season
                      </Button>
                    ) : (
                      <Button
                    ...
                    <Button asChild size="sm" variant="ghost">
                      <a href={`/seasons/${slugify((c.title || "").trim())}`}>
                        Overview
                      </a>
                    </Button>
                  </div>
