...
export default async function SeasonOverviewPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <section className="space-y-4">
      <header>
        <h1 className="text-2xl font-bold" style={{ color }}>{frontmatter.title || slugifyTitle(slug) || 'Season'}</h1>
        <div className="text-sm text-muted-foreground">{frontmatter.sector ? `Sector: ${frontmatter.sector}` : null} {frontmatter.tone ? `· Tone: ${frontmatter.tone}` : null}</div>
      </header>
