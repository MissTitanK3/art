...
export async function GET(req: NextRequest) {
  ...
  if (currentShipId) {
    ...
    const hasChiefEngineer = (staffed || []).some((s: any) => (s as any)?.position?.id === 'chief_engineer');
    const hasNavChief = (staffed || []).some((s: any) => (s as any)?.position?.id === 'navigator_in_chief');
    const hasCommsLead = (staffed || []).some((s: any) => (s as any)?.position?.id === 'comms_lead');
    const hasCMO = (staffed || []).some((s: any) => (s as any)?.position?.id === 'chief_medical_officer');
    const hasSecurityChief = (staffed || []).some((s: any) => (s as any)?.position?.id === 'security_chief');
    const hasCSO = (staffed || []).some((s: any) => (s as any)?.position?.id === 'chief_science_officer');
    ...
    for (const s of (staffed as any[]) || []) {
      ...
      if (Object.keys(crewContrib).length > 0)
        breakdown.items.push({ type: 'crew', id: String(crew.id || ''), name: crew.name, contributions: crewContrib });
      if (Object.keys(posContrib).length > 0)
        breakdown.items.push({ type: 'position', id: String(pos.id || ''), name: pos.name, contributions: posContrib });
    }

    // Department head aura: Chief Engineer boosts engineering-related effects by +20%
    if (hasChiefEngineer) {
      const keys: (keyof typeof bonuses)[] = ['repair_bonus', 'integrity_upkeep'];
  ...
  return NextResponse.json({ base: base || null, bonuses, breakdown, ship_id: currentShipId });
}
