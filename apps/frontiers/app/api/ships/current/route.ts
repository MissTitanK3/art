...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase
    .from("profile_ships")
    .select(
      "profile_id, ship_id, acquired_at, updated_at, ship:ship_catalog(*)",
    )
...
export async function POST(req: Request) {
  ...
  if (!profile_id || !ship_id)
    return NextResponse.json(
      { error: "profile_id and ship_id required" },
      { status: 400 },
    );
  ...
  if (!cat)
    return NextResponse.json({ error: "Unknown ship_id" }, { status: 400 });
  ...
  if (seed_components) {
    try {
      ...
      if (!existing || existing.length === 0) {
        ...
        const wants: Array<{ slot: string; kind?: string | null }> =
          Object.entries(base).map(([slot, kind]) => ({
            slot,
            kind,
          }));
        const slots = wants.length
          ? wants.map((w) => w.slot)
          : ["hull", "engine", "comms", "aux"];
        ...
        const rows = (
          wants.length
            ? wants
            : slots.map((s) => ({ slot: s, kind: null as string | null }))
        )
          .map(({ slot, kind }) => {
            const list = bySlot.get(slot) || [];
            const pick =
              kind && list.some((x) => x.id === kind)
                ? kind
                : list.sort((a, b) => a.tier - b.tier)[0]?.id || null;
            return pick
              ? { profile_id, slot, kind: pick, level: 1, integrity: 1 }
              : null;
          })
