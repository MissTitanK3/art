...
export async function POST(req: Request) {
  ...
  if (!current) return NextResponse.json({ error: 'No ship selected' }, { status: 400 });

  // 1) Clear current selection
  const { error: e1 } = await supabase.from('profile_ships').delete().eq('profile_id', profile_id);
  ...
  try {
    ...
    const { data: s } = await supabase.from('ship_states').select('*').eq('profile_id', profile_id).maybeSingle();
    if (s) {
      await supabase
        .from('ship_states')
        .update({ morale: Math.min(1, Number(s.morale || 0) + 0.1), last_update: new Date().toISOString() })
        .eq('profile_id', profile_id);
    } else {
      await supabase.from('ship_states').insert({ profile_id, ship_condition: 1, morale: 1, fatigue: 0 });
    }
    ...
    for (const r of [...(peers1 || []), ...(peers2 || [])]) {
      ...
      if (next !== cur) {
        updates.push({ source_id: a, target_id: b, relation: 'crew', trust: next });
      }
    }
    if (updates.length) {
      await supabase.from('connections_v2').upsert(updates, { onConflict: 'source_id,target_id' });
    }
  } catch {}

  // Cost: small fatigue increase as disruption
  try {
    const { data: s } = await supabase.from('ship_states').select('fatigue').eq('profile_id', profile_id).maybeSingle();
    if (s) {
      await supabase
        .from('ship_states')
        .update({
