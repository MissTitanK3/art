...
export async function POST(req: Request) {
  ...
  let { trust } = body
  if (!source_id || !target_id || typeof trust !== 'number') return NextResponse.json({ error: 'Missing fields' }, { status: 400 })
  trust = Math.max(0, Math.min(1, trust))

  if (!(isUUID(source_id) && isUUID(target_id))) {
    return NextResponse.json({ ok: false, warning: 'IDs must be UUIDs for trust graph' }, { status: 200 })
  }
