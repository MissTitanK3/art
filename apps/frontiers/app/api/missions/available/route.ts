...
export async function GET(req: NextRequest) {
  ...
  if (shipId) {
    ...
    for (const s of staffed || []) {
      if ((s as any).crew_id) staffedPositions.add(String((s as any).position_id));
    }
    // Ship tier
  ...
  const available = (missions || []).map((m) => {
    ...
    const reqPos: string[] = Array.isArray(ra.requires_positions) ? ra.requires_positions.map(String) : [];
    const anyOf: string[] = Array.isArray(ra.any_of_positions) ? ra.any_of_positions.map(String) : [];
    const minTier: number | null = typeof ra.min_ship_tier === 'number' ? ra.min_ship_tier : null;
    const reqComps: Array<{ slot?: string; kind?: string; level?: number }> = Array.isArray(ra.required_components)
      ? ra.required_components
      : [];

    let ok = true;
    if (ok && reqPos.length > 0) ok = reqPos.every((p) => staffedPositions.has(p));
    ...
    return { id: (m as any).id, title: (m as any).title, description: (m as any).description, available: ok };
  });
