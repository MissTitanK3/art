...
export function FleetContainer({ profileId }: { profileId: string | null }) {
  ...
  const {
    ...
    selectShip,
    abandonShip,
  } = useShipCatalog(profileId);
  // Ship state
  const { shipLoading, ship, resetting, onDockRepair } =
    useShipState(profileId);
  // Components
  const {
    componentsLoading,
    components,
  ...
  const installComponentForSlot = async (slot: ShipComponent["slot"]) => {
    ...
    if (!kind) {
      toast.error("No available component for this slot");
      return;
    }
    ...
    const calc = (k?: any) => {
      ...
      if (lvl > 1)
        for (const [kk, vv] of Object.entries(per))
          out[kk] = (out[kk] || 0) + (Number(vv) || 0) * (lvl - 1);
      return out;
    };
    const a = calc(curK);
    const b = calc(nextK);
  ...
  const { hiredCrew, hiredLoading, hireCrew, fireCrew } =
  ...
  } = useStaffing(profileId, currentShip);
  ...
  const uncoveredNeeds = React.useMemo(() => {
    ...
    for (const p of positionTemplates) {
      ...
      if (assigned === 0 || (p.required && assigned === 0))
        needs.add(p.position_id);
    }
    return needs;
  ...
  const groupedMarket = React.useMemo(() => {
    ...
    for (const [k, arr] of m) {
      const scoreFor = (c: CrewCatalog) =>
        computeCrewFit(c, autoStrategy, uncoveredNeeds).score;
      arr.sort((a, b) => {
        if (marketBestFit) {
          const sa = scoreFor(a),
            sb = scoreFor(b);
  ...
  const { derivedBonuses, derivedBreakdown } = useDerivedBonuses(profileId, [
    assignments,
  ]);
    ...
    <main className="mx-auto max-w-5xl space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)}>
        <TabsList className="w-full h-auto flex-wrap justify-between gap-2">
          <TabsTrigger
            className="basis-[calc(50%-0.375rem)] sm:basis-auto sm:flex-1"
            value="current"
          >
            Current Ship & Hired Crew
          </TabsTrigger>
          <TabsTrigger
            className="basis-[calc(50%-0.375rem)] sm:basis-auto sm:flex-1"
            value="staffing"
