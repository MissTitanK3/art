  ...
  const columns = useMemo<ColumnDef<Donation>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (info) => counts[info.getValue() as string] || 0,
      },
      {
        header: "When",
            ...
            <div key={s.region_id || "unknown"} className="rounded border p-3">
              ...
              <div className="mt-2 h-2 bg-muted rounded overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, Math.round((Number(s.total || 0) / Math.max(1, maxTotal(summary))) * 100))}%`,
                  }}
                />
              </div>
            </div>
