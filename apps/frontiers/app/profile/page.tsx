    ...
    <section className="space-y-6">
      <header>
        ...
        <div className="text-sm text-muted-foreground">
          {profile?.display_name || "Anonymous"} · XP {xp}
        </div>
      </header>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {!profile && (
            <Button variant="secondary" onClick={ensureProfile}>
        ...
        <CardContent className="space-y-3">
          <div className="text-sm text-muted-foreground">
            {Number.isFinite(dock_lat) && Number.isFinite(dock_lng) ? (
              <div>
                Dock set at ~ {(dock_lat as number).toFixed(4)},{" "}
                {(dock_lng as number).toFixed(4)} · radius{" "}
                {(dock_radius_km ?? 0.4023).toFixed(4)} km
              </div>
            ) : (
              <div>No dock set.</div>
            )}
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="secondary" onClick={setDockHere}>
              Set Dock to my location
            </Button>
            <Button
              size="sm"
              variant="outline"
          ...
          <div className="text-[11px] text-muted-foreground">
            Rest bonus applies within 0.4023 km of Dock.
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {defs.map((a) => {
              ...
              const badge = a.reward?.badge
                ? BADGE_EMOJI[a.reward.badge] || "🎖️"
                : "🎖️";
              return (
                <li
                  key={a.id}
                  className={`border rounded p-2 ${earned ? "bg-primary/5 border-primary/40" : "opacity-60"}`}
                  title={a.title}
                >
