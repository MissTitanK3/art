...
export function NotificationsRealtime() {
  ...
  useEffect(() => {
    ...
    broadcast.on('broadcast', { event: 'notify' }, (payload) => {
      const { title, body, level, channel: ch, link } = (payload?.payload ?? {}) as any;
      if (!title) return;
    ...
    if (userId) {
      ...
      (async () => {
        const { data } = await supabase
          ...
          .select('notification_id, title, body, level, channel, link, sticky, expires_at, read_at, dismissed_at')
          .eq('user_id', userId)
        ...
        for (const row of data ?? []) {
          ...
          const ttlMs = row.expires_at ? new Date(row.expires_at as any).getTime() - Date.now() : undefined;
          const newId = notify({
            id,
      ...
      dbSub = supabase
        ...
        .on(
          ...
          async (msg) => {
            ...
            const already = notificationsStore.getState().items.find((i) => i.id === row.notification_id);
            if (already) return;
            const { data } = await supabase
              .from('notifications')
              .select('id, title, body, level, channel, link, sticky, expires_at')
              .eq('id', row.notification_id)
              .single();
            if (!data) return;
            const ttlMs = data.expires_at ? new Date(data.expires_at as any).getTime() - Date.now() : undefined;
            notify({
              id: data.id,
        ...
        .on(
          'postgres_changes',
          { event: 'UPDATE', schema: 'public', table: 'notification_recipients', filter: `user_id=eq.${userId}` },
          (msg) => {
            const row = (msg as any).new as { notification_id: string; read_at?: string | null; dismissed_at?: string | null };
            if (!row?.notification_id) return;
            if (row.dismissed_at) {
      ...
      try {
        ...
        for (const [id] of prevMap) {
          if (!currentIds.has(id)) {
            ...
            void supabase.rpc('dismiss_notification', { p_notification_id: id as any }).then(({ data, error }) => {
              if (error) console.warn('dismiss_notification failed', error.message);
              else if (!data) console.warn('dismiss_notification no-op for', id);
            });
          }
        }
        ...
        for (const item of state.items) {
          ...
          if (was && !was.readAt && item.readAt) {
            void supabase.rpc('mark_notification_read', { p_notification_id: item.id as any }).then(({ data, error }) => {
              if (error) console.warn('mark_notification_read failed', error.message);
              else if (!data) console.warn('mark_notification_read no-op for', item.id);
            });
          }
        }
      } catch {}
