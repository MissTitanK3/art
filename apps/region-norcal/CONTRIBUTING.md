# 🗺️ Contributing Guide for `apps/region-norcal`
...
1. **Keep region logic here**  
   ...
   - Any DB or API adapters should live in `apps/region-norcal/lib/`.  

2. **Separation of concerns**  
   - `@workspace/store` = business logic & state  
   - `@workspace/ui` = reusable presentation components  
   - `apps/region-norcal` = glue code + region-specific DB/backend  
...
- ❌ Don’t copy Zustand stores into `apps/region-norcal`. They belong in `@workspace/store`.  
