...
export function determineCourseStatus(
  ...
  if (
    completed > 0 &&
    completed >= Math.max(1, Math.round(memberProgress.length * 0.6))
  ) {
    return "completed";
  }
...
export function convertPodsToMemberProgress(
  pods: Pod[],
): AcademyMemberProgress[] {
...
export function buildInstructorProfiles(
  pods: Pod[],
): AcademyInstructorProfile[] {
  ...
  for (const pod of pods) {
    for (const member of pod.team) {
      ...
      const dispatchCertified = member.certs.some((cert) =>
        cert.id.startsWith("dispatch-"),
      );
      ...
      const focusSource =
        member.profile.field_roles?.[0] ??
        member.skills?.[0] ??
        member.profile.affiliation ??
        "Operational Support";
      const focus = humanizeLabel(
        typeof focusSource === "string" ? focusSource : String(focusSource),
      );
      const hasExpiredCert = member.certs.some(
        (cert) => cert.level === "expired",
      );
      const hasCompletedCert = member.certs.some(
        (cert) => cert.level === "completed" || cert.level === "mentor",
      );
      const vettingStatus: AcademyInstructorProfile["vettingStatus"] =
        hasExpiredCert
          ? "needs_review"
          : hasCompletedCert
            ? "cleared"
  ...
  return groups.map((group) => ({
    ...
    courses: group.courses.map((course) => {
      ...
      return {
        ...
        status: determineCourseStatus(
          "certId" in course ? course.certId : undefined,
          members,
        ),
      };
    }),
...
export function buildTrainingSessions(
  ...
  const first = Array.isArray(dataOrInstructors)
    ? dataOrInstructors[0]
    : undefined;
  const looksLikeData =
    !!first &&
    ("slug" in (first as any) ||
      "classId" in (first as any) ||
      "status" in (first as any));
  ...
  for (let i = 0; i < count; i++) {
    const instructor =
      instructors.length > 0 ? instructors[i % instructors.length] : undefined;
