...
function extractCourseInterests(profile: any, member: any): string[] {
  ...
  if (profile) {
    addValue(profile.academy_interests);
    addValue(profile.course_interests);
    addValue(profile.interested_courses);
    addValue(profile.interests?.academy);
    addValue(profile.interests?.courses);
    addValue(profile.interests);
    addValue(profile.preferred_courses);

    if (Array.isArray(profile.self_status_flags)) {
      for (const raw of profile.self_status_flags as unknown[]) {
        if (typeof raw !== 'string') continue;
        const trimmed = raw.trim();
        if (trimmed.length === 0) continue;
        const match = trimmed.match(/^(?:academy|course)_interest[:=_-](.+)$/i);
        if (match?.[1]) {
          addValue(match[1]);
          continue;
        }
        const genericMatch = trimmed.match(/^interest[:=_-](.+)$/i);
        if (genericMatch?.[1]) {
          addValue(genericMatch[1]);
        }
      }
    }
  }
  ...
  if (member) {
    addValue((member as any).interests);
    addValue((member as any).academy_interests);
    addValue((member as any).course_interests);
  }
  ...
  for (const pod of pods) {
    for (const member of pod.team) {
      ...
      results.push({
        ...
        interestedCourses: interestedCourses.length > 0 ? interestedCourses : undefined,
      });
    }
  }
  return results;
...
export function buildTrainingSessions(
  ...
  const looksLikeData =
    !!first && ('slug' in (first as any) || 'classId' in (first as any) || 'status' in (first as any));
