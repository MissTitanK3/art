...
import CourseSidebarList, { CourseSidebarGroup } from '@/components/client/CourseSidebarList';

export default async function DocsSidebarServer() {
  const allCourses = getAllCourses();
  const courseMap: Record<string, { title: string; version: number | string }> = Object.fromEntries(
    allCourses.map((c) => [
      c.slug,
      { title: c.frontmatter.title ?? c.slug, version: c.frontmatter.version ?? '' },
    ])
  );
