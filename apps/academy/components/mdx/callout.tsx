...
import { Info, AlertCircle, CheckCircle2, Lightbulb, Flame, BookOpenCheck } from 'lucide-react';

export type CalloutType = 'info' | 'warning' | 'success' | 'hint' | 'important' | 'rabbit-hole';
...
> = {
  info: {
    icon: <Info className="w-11 h-11 text-blue-800" />,
    label: 'Info',
  ...
  warning: {
    icon: <AlertCircle className="w-11 h-11 text-yellow-800" />,
    label: 'Warning',
  ...
  success: {
    icon: <CheckCircle2 className="w-11 h-11 text-green-800" />,
    label: 'Success',
  ...
  hint: {
    icon: <Lightbulb className="w-11 h-11 text-purple-800" />,
    label: 'Hint',
  ...
  important: {
    icon: <Flame className="w-11 h-11 text-red-800" />,
    label: 'Important',
  ...
  'rabbit-hole': {
    icon: <BookOpenCheck className="w-11 h-11 text-gray-800" />,
    label: 'Rabbit Hole',
...
export function Callout({ type = 'info', children }: { type?: CalloutType; children: ReactNode }) {
  ...
  return (
    <div className={`flex ${style.border} ${style.bg} rounded p-4`}>
      <div className="mr-3 -ml-[66px] mt-1 flex items-center justify-center w-11">{style.icon}</div>
