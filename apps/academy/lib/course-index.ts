...
export function getPrevNextWithMetadata(slug: string) {
  ...
  const courseMap = Object.fromEntries(
    allCourses.map((c) => [c.slug, c.frontmatter]),
  );
  ...
  return {
    prev: prevSlug
      ? { slug: prevSlug, title: courseMap[prevSlug]?.title }
      : null,
    next: nextSlug
      ? { slug: nextSlug, title: courseMap[nextSlug]?.title }
      : null,
  };
}

export function getPrevNext(slug: string) {
  const flat: { slug: string; label: string }[] = COURSE_GROUPS.flatMap(
    (group) =>
      group.courses.map((c) => ({
        slug: typeof c === "string" ? c : c.slug,
        label: group.label,
