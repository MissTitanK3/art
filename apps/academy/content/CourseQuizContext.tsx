...
export function QuizProvider({ children, slug }: { children: React.ReactNode; slug: string }) {
  const [passed, setPassedState] = useState(false);
  const [answers, setAnswersState] = useState<Record<string, string | string[]>>({});
  ...
  return <QuizContext.Provider value={{ passed, setPassed, answers, setAnswers }}>{children}</QuizContext.Provider>;
}
